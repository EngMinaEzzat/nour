...
## 2025-02-27 - [CRITICAL] Prevented SSRF bypass via IPv4-mapped IPv6
...
## 2026-06-30 - Fix SSRF bypass via IPv4-mapped IPv6 evaluation
**Vulnerability:** The `isPrivateIp` function for SSRF protection manually checked `addr.isIPv4MappedAddress()` using `ipaddr.parse()`, which failed to correctly identify partially compressed or non-standard IPv4-mapped IPv6 formats (like `0::ffff:127.0.0.1` or `::ffff:7f00:1`), bypassing loopback and private IP blocklists.
**Learning:** Manual traversal and checking of IPv6 to IPv4 address conversions using `ipaddr.parse(ip)` leaves room for unhandled structural variations of mapped addresses.
**Prevention:** For SSRF blocklist validation, strictly use `ipaddr.process(ip)` which seamlessly unifies and normalizes all representations of IPv4-mapped IPv6 addresses into standard IPv4 structures before running `.range()` checks, eliminating bypass vectors.
