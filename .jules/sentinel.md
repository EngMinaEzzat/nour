...
## 2024-05-24 - [Fix SSRF bypass via uncompressed IPv4-mapped IPv6 representations]
**Vulnerability:** The IP validation logic only strictly matched the abbreviated prefix `::ffff:` for IPv4-mapped IPv6 addresses, failing to catch fully or partially uncompressed forms (e.g., `0:0:0:0:0:ffff:127.0.0.1` or `0::ffff:127.0.0.1`). This allowed an attacker to bypass the SSRF protection logic by formatting a loopback IP using an uncompressed representation.
**Learning:** Naively using regex like `/^(?:::|(?:0{1,4}:){5})ffff:(.+)$/` on IP validation fails to capture valid partially compressed forms of IPv6 addresses since standard libraries will normalize them but direct string matching will fail. We had to implement normalisation logic by expanding the `::` block before matching the prefix using our regex.
**Prevention:** Rather than directly manipulating IP address formatting via string operations, it's necessary to normalize the IP string by fully expanding any `::` compressed segments to 8 blocks before regex matching, or ideally rely on a dedicated, tested library like `ipaddr.js` for IP parsing and type checking.
