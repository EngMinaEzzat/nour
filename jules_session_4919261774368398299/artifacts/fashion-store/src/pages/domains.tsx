...
const STATUS_CONFIG: Record<string, { label: string; color: string; icon: any }> = {
  PENDING_DNS: { label: t("text_5b499f2f", "في انتظار DNS"), color: "text-amber-600 bg-amber-50", icon: Clock },
  VERIFYING: { label: t("text_530598d8", "جاري التحقق"), color: "text-blue-600 bg-blue-50", icon: Clock },
  ACTIVE: { label: t("text_ca4bcb57", "نشط ومفعّل"), color: "text-green-600 bg-green-50", icon: CheckCircle },
  FAILED: { label: t("text_8a8fce5a", "فشل التحقق"), color: "text-red-600 bg-red-50", icon: AlertTriangle },
  REMOVED: { label: t("text_19c1d1f2", "تمت الإزالة"), color: "text-gray-500 bg-gray-100", icon: AlertTriangle },
};
  ...
  return (
    <div className="space-y-6" dir="rtl">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
        <h1 className="text-2xl font-bold text-gray-900">{t("text_1b3b4e81", t("text_1b3b4e81", "النطاق المخصص"))}</h1>
        <p className="text-gray-500 mt-1">{t("text_532f2317", t("text_532f2317", "اربط نطاقك الخاص بمتجرك على نور"))}</p>
      </motion.div>
      ...
      {isLocked && (
        <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }}
          ...
          <h2 className="text-lg font-bold text-purple-900 mb-2">{t("text_ce59de87", t("text_ce59de87", "متاح في خطة برو"))}</h2>
          <p className="text-purple-700 text-sm mb-4">{t("text_909cc374", t("text_909cc374", "ربط النطاق المخصص متاح فقط لمشتركي خطة برو"))}</p>
          <a href="/billing" className="inline-flex items-center gap-2 bg-purple-600 text-white px-5 py-2.5 rounded-xl font-medium hover:bg-purple-700 transition-colors text-sm">
            <TrendingUp className="w-4 h-4" /> {t("text_59f39deb", t("text_59f39deb", "ترقية إلى برو"))}
                                </a>
        </motion.div>
      )}
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
          ...
          {domain.status === "PENDING_DNS" && (
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 space-y-3">
              <p className="text-sm font-semibold text-amber-800">{t("text_74085dc2", t("text_74085dc2", "أضف السجلات التالية في لوحة إدارة DNS الخاصة بك:"))}</p>
          ...
          {domain.status === "ACTIVE" && (
            <a href={`https://${domain.domain}`} target="_blank" rel="noopener noreferrer"
              ...
              <ExternalLink className="w-4 h-4" /> {t("text_b6970247", t("text_b6970247", "زيارة المتجر"))}
                                      </a>
          )}
        </motion.div>
      )}
      ...
      {!isLocked && !isLoading && (!domain || domain.status === "REMOVED") && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className="flex items-center gap-3 mb-5">
            ...
            <h2 className="text-base font-semibold text-gray-800">{t("text_22bd6107", t("text_22bd6107", "ربط نطاق جديد"))}</h2>
          </div>
          <div className="space-y-3">
            <input
              ...
              placeholder={t("text_0f50d66c", "متجري.com")}
            ...
            >
              {requestMutation.isPending ? t("text_22dff91c", "جاري الإرسال...") : t("text_8e59f946", "ربط النطاق")}
            </button>
          </div>
          <p className="text-xs text-gray-400 mt-3">{t("text_fdd1c7a4", t("text_fdd1c7a4", "ستحتاج إلى الوصول لإعدادات DNS لنطاقك لإتمام عملية الربط"))}</p>
        </motion.div>
      )}
        ...
        {showConfirmRemove && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }}
              ...
              <h3 className="font-bold text-gray-900 mb-2">{t("text_ea342f07", t("text_ea342f07", "تأكيد الإزالة"))}</h3>
              <p className="text-sm text-gray-600 mb-5">{t("text_ac9869d9", t("text_ac9869d9", "هل أنت متأكد من إزالة النطاق المخصص؟ سيتوقف المتجر عن العمل على هذا النطاق."))}</p>
              <div className="flex gap-3">
                ...
                <button onClick={() => removeMutation.mutate()} disabled={removeMutation.isPending}
                  ...
                  {removeMutation.isPending ? t("text_e978b4f4", "جاري الإزالة...") : t("text_eed790a8", "إزالة")}
                </button>
              </div>
            </motion.div>
          </motion.div>
