    ...
    mutationFn: (id: number) =>
      fetch(api(`/abandoned-carts/${id}/remind`), { method: "POST", credentials: "include" })
        .then(async (r) => {
          ...
          if (!r.ok) throw new Error(d.error ?? t("text_a838e35c", "فشل"));
  ...
  return (
    <div className="container mx-auto px-4 py-8 pb-24 max-w-5xl">
      <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-foreground">{t("text_d2173ddf", t("text_d2173ddf", "سلات التسوق المتروكة"))}</h1>
          <p className="text-muted-foreground mt-1">{t("text_a2402be5", t("text_a2402be5", "تابع العملاء الذين أضافوا منتجات ولم يكملوا الشراء"))}</p>
        </div>

        {/* Stats */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
          {[
            { label: t("text_e6b75acd", "سلات متروكة"), value: stats?.totalAbandoned ?? 0, icon: TrendingDown, color: "text-destructive" },
            { label: t("text_0a812de1", "قيمة ضائعة (ج.م)"), value: (stats?.totalValue ?? 0).toLocaleString("ar-EG"), icon: DollarSign, color: "text-amber-600" },
            { label: t("text_93be6266", "لديهم واتساب"), value: stats?.withPhone ?? 0, icon: MessageCircle, color: "text-green-600" },
            { label: t("text_7c6f1752", "نشطة الآن"), value: stats?.totalActive ?? 0, icon: ShoppingCart, color: "text-primary" },
          ].map((s) => (
        ...
        <Tabs value={tab} onValueChange={(v) => setTab(v as typeof tab)} className="mb-6">
          <TabsList className="rounded-2xl">
            <TabsTrigger value="abandoned" className="rounded-xl gap-1.5">
              {t("text_1f479f59", t("text_1f479f59", "متروكة"))}
            ...
            <TabsTrigger value="active" className="rounded-xl">{t("text_2c7cd087", t("text_2c7cd087", "نشطة"))}</TabsTrigger>
          </TabsList>
        </Tabs>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-24 text-muted-foreground bg-muted/20 rounded-3xl border border-dashed border-border">
            ...
            <p className="text-xl font-semibold mb-1">
              {tab === "abandoned" ? t("text_acb9322f", "لا توجد سلات متروكة") : t("text_0e780257", "لا توجد سلات نشطة حالياً")}
            </p>
            <p className="text-sm">
              {tab === "abandoned"
                ? t("text_7132b645", "ممتاز! جميع العملاء أكملوا طلباتهم")
                : t("text_749c47a3", "لا يوجد عملاء في منتصف التسوق الآن")}
            </p>
          </div>
        ) : (
                        ...
                        <div className="flex items-start gap-4">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-3 flex-wrap mb-2">
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <div>
                                  <p className="font-semibold text-sm">
                                    {cart.customerName ?? <span className="text-muted-foreground italic">{t("text_2b0d4474", t("text_2b0d4474", "زائر مجهول"))}</span>}
                                  </p>
                                  {cart.customerEmail && (
                              ...
                              >
                                {cart.status === "abandoned" ? t("text_1f479f59", "متروكة") : t("text_2c7cd087", "نشطة")}
                              </Badge>
                            ...
                            <div className="flex items-center gap-2 mb-2 flex-wrap">
                              ...
                              {itemsList.length > 3 && (
                                <span className="text-xs text-muted-foreground">+{itemsList.length - 3} {t("text_684b0d48", t("text_684b0d48", "منتجات"))}</span>
                              )}
                              {itemsList.length === 0 && (
                                <span className="text-xs text-muted-foreground italic">{t("text_f34cb15a", t("text_f34cb15a", "سلة فارغة"))}</span>
                              )}
                            </div>
                            ...
                            <div className="flex items-center gap-4 text-xs text-muted-foreground flex-wrap">
                              ...
                              {!cart.customerPhone && !cart.customerEmail && (
                                <span className="flex items-center gap-1 text-muted-foreground/60 italic">
                                  <AlertCircle className="w-3 h-3" /> {t("text_bdcf1105", t("text_bdcf1105", "لا تفاصيل تواصل"))}
                                                                                    </span>
                              )}
                            </div>
                          </div>
                          ...
                          <div className="flex flex-col items-end gap-2 shrink-0">
                            <p className="text-lg font-bold text-foreground">
                              {cart.totalAmount.toLocaleString("ar-EG")} {t("text_3c111129", t("text_3c111129", "ج.م"))}
                                                                        </p>
                            <p className="text-[11px] text-muted-foreground">{cart.itemCount} {t("text_bb7632a2", t("text_bb7632a2", "منتج"))}</p>
                            ...
                            <div className="flex items-center gap-1.5 mt-1">
                              {cart.customerPhone ? (
                                ...
                                >
                                  ...
                                  {t("text_0d8c9bb4", t("text_0d8c9bb4", "واتساب"))}
                                ...
                                <Button size="sm" variant="outline" className="rounded-xl h-8 text-xs" disabled>
                                  ...
                                  {t("text_38a3a4e9", t("text_38a3a4e9", "لا رقم"))}
                                                                                        </Button>
                              )}
          ...
          >
            ...
            <div>
              <p className="font-semibold">
                {t("text_58bd6a1a", t("text_58bd6a1a", "يمكنك الوصول إلى"))} {stats?.withPhone} {t("text_f771d506", t("text_f771d506", "عميل عبر واتساب"))}
                                            </p>
              <p className="text-green-700/80 mt-0.5">
                {t("text_7c3fbe3b", t("text_7c3fbe3b", "اضغط على \"واتساب\" بجانب أي سلة لفتح رسالة تذكير جاهزة — سيفتح واتساب ويب مباشرةً في متصفحك"))}
                                            </p>
            </div>
          </motion.div>
        )}
