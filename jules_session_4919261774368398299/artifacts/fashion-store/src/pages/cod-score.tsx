...
function getRiskBadge(risk: CodCustomer["riskLevel"]) {
  if (risk === "high")   return <Badge className="bg-red-100 text-red-700 border-red-200 text-[10px]">{t("text_416ed6f5", t("text_416ed6f5", "خطر مرتفع"))}</Badge>;
  if (risk === "medium") return <Badge className="bg-yellow-100 text-yellow-700 border-yellow-200 text-[10px]">{t("text_14184253", t("text_14184253", "متوسط"))}</Badge>;
  return                        <Badge className="bg-green-100 text-green-700 border-green-200 text-[10px]">{t("text_71d8a702", t("text_71d8a702", "موثوق"))}</Badge>;
}
...
export default function CodScore() {
  ...
  return (
    <div className="space-y-6 pb-10" dir="rtl">
      <div>
        <h1 className="text-xl font-bold">{t("text_e3a2dbd3", t("text_e3a2dbd3", "تقييم عملاء الدفع عند الاستلام"))}</h1>
        <p className="text-sm text-muted-foreground mt-1">{t("text_1346a57f", t("text_1346a57f", "معدل تأكيد الطلبات لكل عميل — لتقليل الطلبات الوهمية وتحديد العملاء الموثوقين"))}</p>
      </div>
          ...
          <>
            <Card className="border-border/50">
              <CardContent className="p-4 text-center">
                ...
                <p className="text-[11px] text-muted-foreground mt-1">{t("text_d53318d9", t("text_d53318d9", "متوسط التأكيد"))}</p>
              </CardContent>
            </Card>
            <Card className="border-border/50">
              <CardContent className="p-4 text-center">
                <p className="text-2xl font-bold text-red-600">{summary?.highRiskCount ?? 0}</p>
                <p className="text-[11px] text-muted-foreground mt-1">{t("text_19401f33", t("text_19401f33", "عملاء خطر مرتفع"))}</p>
              </CardContent>
            </Card>
            <Card className="border-border/50">
              <CardContent className="p-4 text-center">
                <p className="text-2xl font-bold">{summary?.totalCodOrders ?? 0}</p>
                <p className="text-[11px] text-muted-foreground mt-1">{t("text_2d328ac2", t("text_2d328ac2", "طلبات COD"))}</p>
              </CardContent>
            </Card>
          </>
        )}
      ...
      <Card className="border-border/50 bg-muted/30">
        <CardContent className="p-4 flex flex-wrap gap-4 text-xs text-muted-foreground">
          <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-green-500 inline-block" />{t("text_a04767c9", t("text_a04767c9", "موثوق: تأكيد ≥ 70%"))}</span>
          <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-yellow-400 inline-block" />{t("text_eeb6f957", t("text_eeb6f957", "متوسط: تأكيد 40–69%"))}</span>
          <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-red-500 inline-block" />{t("text_c4f14f8a", t("text_c4f14f8a", "خطر مرتفع: تأكيد &lt; 40%"))}</span>
        </CardContent>
      </Card>
        ...
        {isLoading ? Array(5).fill(0).map((_, i) => <Skeleton key={i} className="h-24 rounded-xl" />) :
         customers.length === 0 ? (
          <div className="text-center py-24">
            ...
            <p className="text-muted-foreground">{t("text_61a86827", t("text_61a86827", "لا توجد طلبات COD بعد"))}</p>
          </div>
         ) : customers.map((c) => (
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex gap-4 text-[11px] text-muted-foreground">
                    <span>{c.totalOrders} {t("text_f48e3aa7", t("text_f48e3aa7", "طلب"))}</span>
                    <span className="text-green-600">{c.confirmedOrders} {t("text_c6310c14", t("text_c6310c14", "مؤكد"))}</span>
                    <span className="text-red-500">{c.cancelledOrders} {t("text_f5eb0776", t("text_f5eb0776", "ملغي"))}</span>
