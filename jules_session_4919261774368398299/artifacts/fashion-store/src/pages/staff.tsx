...
export default function Staff() {
  ...
  async function handleInvite(e: React.FormEvent) {
    ...
    try {
      ...
      setError(msg ?? t("text_871ef154", "حدث خطأ أثناء الإضافة"));
    }
  }
  ...
  async function handleUpdateRole() {
    ...
    try {
      ...
      const msg = (err as { response?: { data?: { error?: string } } })?.response?.data?.error;
      setError(msg ?? t("text_6f2c1f78", "حدث خطأ أثناء التحديث"));
    }
  }
  ...
  async function handleDelete() {
    ...
    try {
      ...
      const msg = (err as { response?: { data?: { error?: string } } })?.response?.data?.error;
      setError(msg ?? t("text_c96a6213", "حدث خطأ أثناء الحذف"));
    }
  }
  ...
  return (
    <div className="container mx-auto px-4 py-10 max-w-4xl">
      ...
      >
        <div>
          <h1 className="text-4xl font-bold text-foreground mb-1 flex items-center gap-3">
            ...
            {t("text_9bc50be8", t("text_9bc50be8", "إدارة الفريق"))}
                                </h1>
          <p className="text-muted-foreground">{t("text_ca30f40c", t("text_ca30f40c", "أضف موظفين ومديرين لمتجرك وحدد صلاحياتهم"))}</p>
        </div>
        {isOwner && (
          <div className="flex gap-2 shrink-0">
            <Button variant="outline" className="rounded-full gap-2" onClick={() => { setInvLinkOpen(true); setInvLinkResult(null); }}>
              <Link2 className="w-4 h-4" /> {t("text_e7bd7854", t("text_e7bd7854", "دعوة برابط"))}
                                      </Button>
            <Button className="rounded-full gap-2" onClick={() => { setInviteOpen(true); setError(null); }}>
              <UserPlus className="w-4 h-4" /> {t("text_e519a187", t("text_e519a187", "إضافة مباشر"))}
                                      </Button>
          </div>
        )}
      </motion.div>
      ...
      >
        {[
          { role: "owner", desc: t("text_99f0fcc9", "وصول كامل — الإعدادات والفوترة وإدارة الفريق") },
          { role: "manager", desc: t("text_68a16788", "إدارة المنتجات والطلبات والعملاء وعرض التقارير") },
          { role: "catalog_manager", desc: t("text_68f9cb05", "إدارة المنتجات والفئات والمخزون فقط") },
          { role: "order_operator", desc: t("text_408a6dd6", "عرض الطلبات وتحديث حالتها ومعالجة الشحن") },
          { role: "marketing_analyst", desc: t("text_4dea9d3c", "عرض التقارير والتحليلات والعملاء فقط") },
          { role: "staff", desc: t("text_b78c683a", "عرض الطلبات وتحديث حالتها وعرض المنتجات فقط") },
        ].map(({ role, desc }) => {
      ...
      <Card className="border-border/50">
        <CardHeader>
          <CardTitle className="text-lg font-bold flex items-center gap-2">
            ...
            {t("text_f1a79583", t("text_f1a79583", "أعضاء الفريق"))}
                                  {staffList && (
              <Badge variant="secondary" className="ms-auto text-xs">{staffList.length} {t("text_b1ed56cf", t("text_b1ed56cf", "عضو"))}</Badge>
            )}
          </CardTitle>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            ...
            <div className="text-center py-12 text-muted-foreground">
              ...
              <p>{t("text_8d459cc2", t("text_8d459cc2", "لا يوجد أعضاء فريق بعد"))}</p>
              {isOwner && (
                <Button variant="outline" size="sm" className="mt-4 rounded-full" onClick={() => setInviteOpen(true)}>
                  <UserPlus className="w-3.5 h-3.5 me-1.5" /> {t("text_81be2787", t("text_81be2787", "أضف أول عضو"))}
                                                      </Button>
              )}
            </div>
          ) : (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {isMe && (
                          <Badge variant="outline" className="text-xs text-primary border-primary/30">{t("text_c6be60f8", t("text_c6be60f8", "أنت"))}</Badge>
                        )}
                      ...
                      <p className="text-xs text-muted-foreground/60 mt-0.5">
                        {t("text_3826ba47", t("text_3826ba47", "انضم"))} {new Date(member.createdAt).toLocaleDateString("ar-EG")}
                      </p>
                    </div>
        ...
        <DialogContent className="sm:max-w-md" dir="rtl">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              {t("text_5a7e9cd1", t("text_5a7e9cd1", "إضافة عضو جديد"))}
                                      </DialogTitle>
            <DialogDescription>
              {t("text_e90dc741", t("text_e90dc741", "أدخل بيانات العضو الجديد وحدد دوره في المتجر"))}
                                      </DialogDescription>
          </DialogHeader>
          <form onSubmit={handleInvite} className="space-y-4 mt-2">
            <div className="space-y-2">
              <Label>{t("text_bfd0d0dd", t("text_bfd0d0dd", "الاسم (اختياري)"))}</Label>
              <Input
                placeholder={t("text_f7b910be", "اسم الموظف")}
                value={form.name}
                onChange={(e) => setForm({ ...form, name: e.target.value })}
              />
            </div>
            <div className="space-y-2">
              <Label>{t("text_b052caae", t("text_b052caae", "البريد الإلكتروني *"))}</Label>
              <Input
                type="email"
            ...
            <div className="space-y-2">
              <Label>{t("text_88fd9f79", t("text_88fd9f79", "كلمة المرور *"))}</Label>
              <Input
                type="password"
                placeholder={t("text_5c83deec", "6 أحرف على الأقل")}
                required
                minLength={6}
                value={form.password}
            ...
            <div className="space-y-2">
              <Label>{t("text_6dcb4c20", t("text_6dcb4c20", "الدور *"))}</Label>
              <Select
                value={form.role}
              ...
              >
                ...
                <SelectContent>
                  <SelectItem value="manager">{t("text_8298fd5e", t("text_8298fd5e", "مدير — إدارة كاملة"))}</SelectItem>
                  <SelectItem value="catalog_manager">{t("text_b819d144", t("text_b819d144", "مدير كتالوج — منتجات ومخزون"))}</SelectItem>
                  <SelectItem value="order_operator">{t("text_4d593592", t("text_4d593592", "موظف طلبات — معالجة الطلبات"))}</SelectItem>
                  <SelectItem value="marketing_analyst">{t("text_93dddbae", t("text_93dddbae", "محلل تسويق — تقارير فقط"))}</SelectItem>
                  <SelectItem value="staff">{t("text_6efac803", t("text_6efac803", "موظف — صلاحيات محدودة"))}</SelectItem>
                </SelectContent>
              </Select>
            </div>
            {error && (
            ...
            <div className="flex gap-3 pt-2">
              <Button type="submit" className="flex-1" disabled={inviteMutation.isPending}>
                {inviteMutation.isPending ? t("text_a50f9c1f", "جارٍ الإضافة...") : t("text_3cfe1ab5", "إضافة العضو")}
              </Button>
      ...
      <Dialog open={!!editMember} onOpenChange={(o) => !o && setEditMember(null)}>
        <DialogContent className="sm:max-w-sm" dir="rtl">
          <DialogHeader>
            <DialogTitle>{t("text_3f80fa7a", t("text_3f80fa7a", "تغيير الدور"))}</DialogTitle>
            <DialogDescription>{t("text_8a3ffab6", t("text_8a3ffab6", "حدد الدور الجديد لهذا العضو"))}</DialogDescription>
          </DialogHeader>
          <div className="space-y-4 mt-2">
            ...
            >
              ...
              <SelectContent>
                <SelectItem value="manager">{t("text_bd4c63dd", t("text_bd4c63dd", "مدير"))}</SelectItem>
                <SelectItem value="catalog_manager">{t("text_6df53b5d", t("text_6df53b5d", "مدير كتالوج"))}</SelectItem>
                <SelectItem value="order_operator">{t("text_66923665", t("text_66923665", "موظف طلبات"))}</SelectItem>
                <SelectItem value="marketing_analyst">{t("text_d30752a4", t("text_d30752a4", "محلل تسويق"))}</SelectItem>
                <SelectItem value="staff">{t("text_841e1ac2", t("text_841e1ac2", "موظف"))}</SelectItem>
              </SelectContent>
            </Select>
            {error && (
              <div className="flex items-center gap-2 text-destructive text-sm bg-destructive/10 px-3 py-2 rounded-lg">
            ...
            <div className="flex gap-3">
              <Button className="flex-1" onClick={handleUpdateRole} disabled={updateRoleMutation.isPending}>
                {updateRoleMutation.isPending ? t("text_bcd34749", "جارٍ الحفظ...") : t("text_871a087a", "حفظ")}
              </Button>
              <Button variant="outline" onClick={() => setEditMember(null)}>{t("text_b9568e86", t("text_b9568e86", "إلغاء"))}</Button>
            </div>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={invLinkOpen} onOpenChange={(o) => { if (!o) { setInvLinkOpen(false); setInvLinkResult(null); } }}>
        <DialogContent className="sm:max-w-md" dir="rtl">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2"><Link2 className="w-5 h-5 text-primary" /> {t("text_e7bd7854", t("text_e7bd7854", "دعوة برابط"))}</DialogTitle>
            <DialogDescription>{t("text_edde212b", t("text_edde212b", "أنشئ رابط دعوة لمدة 7 أيام يُرسل للعضو عبر أي قناة"))}</DialogDescription>
          </DialogHeader>
          {!invLinkResult ? (
            <div className="space-y-4 mt-2">
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                <Label>{t("text_16479210", t("text_16479210", "الدور"))}</Label>
              ...
              <div className="flex gap-3">
                <Button className="flex-1" disabled={!invLinkForm.email || sendInvLinkMutation.isPending} onClick={() => sendInvLinkMutation.mutate(invLinkForm)}>
                  {sendInvLinkMutation.isPending ? t("text_a05d220c", "جاري الإنشاء...") : t("text_fe4f0fed", "إنشاء رابط الدعوة")}
                </Button>
                <Button variant="outline" onClick={() => setInvLinkOpen(false)}>{t("text_b9568e86", t("text_b9568e86", "إلغاء"))}</Button>
              </div>
            </div>
          ) : (
            <div className="space-y-4 mt-2">
              <div className="bg-green-50 border border-green-200 rounded-xl p-4">
                <p className="text-sm font-medium text-green-800 mb-2">{t("text_8a6f86da", t("text_8a6f86da", "تم إنشاء رابط الدعوة!"))}</p>
              ...
              <div className="flex gap-3">
                <Button className="flex-1" onClick={() => { navigator.clipboard.writeText(`${window.location.origin}${invLinkResult}`); setCopiedInvLink(true); setTimeout(() => setCopiedInvLink(false), 2000); }}>
                  {copiedInvLink ? t("text_be10bcdc", "تم النسخ ✓") : t("text_0d8af0ab", "نسخ الرابط")}
                </Button>
                <Button variant="outline" onClick={() => { setInvLinkOpen(false); setInvLinkResult(null); }}>{t("text_9932cca0", t("text_9932cca0", "إغلاق"))}</Button>
              </div>
            </div>
          )}
        </DialogContent>
        ...
        <Card className="border-border/50 mt-6">
          <CardHeader>
            <CardTitle className="text-base flex items-center gap-2">
              <Mail className="w-4 h-4 text-primary" /> {t("text_eff78ab4", t("text_eff78ab4", "الدعوات المعلّقة"))}
                                        <Badge variant="secondary" className="ms-auto text-xs">{(invitations as any[]).filter((i: any) => i.status === "pending").length} {t("text_5e7972ed", t("text_5e7972ed", "معلّقة"))}</Badge>
            </CardTitle>
          </CardHeader>
              ...
              <div key={inv.id} className="flex items-center justify-between py-3 first:pt-0 last:pb-0 gap-3">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">{ROLE_LABELS[inv.role] ?? inv.role} {t("text_38d97e7f", t("text_38d97e7f", "• تنتهي"))} {new Date(inv.expiresAt).toLocaleDateString("ar-EG")}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${INV_STATUS_COLORS[inv.status] ?? "bg-gray-100 text-gray-600"}`}>{inv.status === "pending" ? t("text_5e7972ed", "معلّقة") : inv.status === "accepted" ? t("text_ef9a104c", "مقبولة") : inv.status === "expired" ? t("text_42c40cf6", "منتهية") : t("text_e92ebe02", "ملغاة")}</span>
                  {inv.status === "pending" && (
        ...
        <DialogContent className="sm:max-w-sm" dir="rtl">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-destructive">
              ...
              {t("text_2cfa4e29", t("text_2cfa4e29", "تأكيد الحذف"))}
                                      </DialogTitle>
            <DialogDescription>
              {t("text_41f446ba", t("text_41f446ba", "هل أنت متأكد من حذف هذا العضو؟ لا يمكن التراجع عن هذا الإجراء."))}
                                      </DialogDescription>
          </DialogHeader>
          {error && (
            <div className="flex items-center gap-2 text-destructive text-sm bg-destructive/10 px-3 py-2 rounded-lg">
          ...
          <div className="flex gap-3 mt-2">
            <Button variant="destructive" className="flex-1" onClick={handleDelete} disabled={removeMutation.isPending}>
              {removeMutation.isPending ? t("text_adfb2130", "جارٍ الحذف...") : t("text_3b9854e1", "حذف")}
            </Button>
            <Button variant="outline" onClick={() => setDeleteId(null)}>{t("text_b9568e86", t("text_b9568e86", "إلغاء"))}</Button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
