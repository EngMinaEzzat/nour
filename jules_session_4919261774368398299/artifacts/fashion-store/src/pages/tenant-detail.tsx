...
const CATEGORY_LABELS: Record<string, string> = {
  ...
  cosmetics: t("text_39363b8c", "تجميل"),
...
const SUB_STATUS_OPTIONS: Array<{ value: string; label: string }> = [
  { value: "trial",     label: t("text_5abde870", "تجريبي") },
  { value: "active",    label: t("text_8caaf953", "نشط") },
  { value: "past_due",  label: t("text_96e0b94d", "متأخر السداد") },
  { value: "suspended", label: t("text_75e3d97e", "موقوف") },
  { value: "canceled",  label: t("text_f5eb0776", "ملغي") },
];
        ...
        <Card className="border-primary/20 bg-gradient-to-br from-primary/5 to-accent/5">
          <CardHeader className="pb-3">
            <CardTitle className="text-sm flex items-center gap-2">
              ...
              {t("text_1b0c4717", t("text_1b0c4717", "إدارة الخطة — مشغّل المنصة"))}
                                      </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <p className="text-xs text-muted-foreground font-medium">{t("text_1b73e0af", t("text_1b73e0af", "الخطة"))}</p>
              ...
              <div className="space-y-1.5">
                <p className="text-xs text-muted-foreground font-medium">{t("text_a0fc3941", t("text_a0fc3941", "حالة الاشتراك"))}</p>
            ...
            <Textarea
              placeholder={t("text_6f592c1a", "ملاحظة داخلية عن التغيير (اختياري)")}
              value={planNote}
            ...
            <div className="flex items-center gap-3">
              ...
              >
                ...
                {updatePlan.isPending ? t("text_bcd34749", "جارٍ الحفظ...") : t("text_02f31ae2", "حفظ التغييرات")}
              </Button>
              <AnimatePresence>
                {planSaved && (
                  <motion.span
                  ...
                  >
                    {t("text_54bb4084", t("text_54bb4084", "✅ تم الحفظ"))}
                                                        </motion.span>
                )}
              </AnimatePresence>
            </div>
          ...
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              <CardTitle className="text-sm flex items-center gap-2">
                ...
                {t("text_e54a0863", t("text_e54a0863", "ملاحظات الدعم"))}
              ...
              >
                ...
                {t("text_67dd6bcb", t("text_67dd6bcb", "إضافة ملاحظة"))}
                                            </Button>
            </div>
          </CardHeader>
          <CardContent className="space-y-3">
            <AnimatePresence>
              {showNoteForm && (
                ...
                >
                  <Textarea
                    placeholder={t("text_cb63e024", "أكتب ملاحظتك هنا...")}
                    value={noteText}
                  ...
                  <div className="flex gap-2">
                    <Button size="sm" onClick={handleAddNote} disabled={createNote.isPending || !noteText.trim()} className="flex-1">
                      {createNote.isPending ? t("text_bcd34749", "جارٍ الحفظ...") : t("text_d417f3f2", "حفظ الملاحظة")}
                    </Button>
                    <Button size="sm" variant="ghost" onClick={() => setShowNoteForm(false)}>{t("text_b9568e86", t("text_b9568e86", "إلغاء"))}</Button>
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
            ...
            {notes && notes.length > 0 ? (
              ...
              !showNoteForm && (
                <p className="text-sm text-muted-foreground text-center py-6">{t("text_18839819", t("text_18839819", "لا توجد ملاحظات بعد"))}</p>
              )
            )}
          </CardContent>
        </Card>
  ...
  if (!tenant) {
    return (
      <div className="container mx-auto px-4 py-24 text-center">
        ...
        <p className="text-muted-foreground">{t("text_a0c401c1", t("text_a0c401c1", "المتجر غير موجود"))}</p>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto px-4 py-10 max-w-5xl">
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                {isPlatformAdmin && tenantAny.planCode && (
                  <Badge className="bg-primary/80 text-white border-primary/40 text-xs">
                    {t("text_8fc75cd7", t("text_8fc75cd7", "خطة"))} {PLAN_LABELS[tenantAny.planCode] ?? tenantAny.planCode}
                  </Badge>
                )}
                {tenant.city && (
      ...
      {stats && (
        ...
        >
          {[
            { label: t("text_b25ceb8b", "المنتجات"), value: stats.totalProducts, icon: Package },
            { label: t("text_889f5569", "الطلبات"), value: stats.totalOrders, icon: ShoppingBag },
            { label: t("text_cd39bdf8", "الإيرادات"), value: `${Number(stats.totalRevenue ?? 0).toLocaleString("ar-EG")} ج.م`, icon: TrendingUp },
          ].map((s) => (
      ...
      {isPlatformAdmin && (
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="mb-10">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-lg font-bold">{t("text_b16ab99b", t("text_b16ab99b", "أدوات المشغّل"))}</h2>
      ...
      {/* Products */}
      <div>
        <h2 className="text-2xl font-bold mb-6">{t("text_c6e2f3c9", t("text_c6e2f3c9", "منتجات المتجر"))}</h2>
        <motion.div
        ...
        {!loadingProducts && !products?.length && (
          <p className="text-muted-foreground text-center py-16">{t("text_268c2228", t("text_268c2228", "لا توجد منتجات في هذا المتجر بعد"))}</p>
        )}
      </div>
    </div>
  );
