...
const PLAN_LABELS: Record<string, string> = {
  starter: t("text_9edb5340", "ستارتر"),
  growth:  t("text_d12e86e1", "جروث"),
  pro:     t("text_eecc506d", "برو"),
};
...
function MerchantStatusToggle({ m, onToggled }: { m: MerchantRow; onToggled: () => void }) {
  ...
  const label = isSuspended ? t("text_daabc826", "إعادة تفعيل") : t("text_e409dbb1", "إيقاف مؤقت");
  ...
  return (
    <AlertDialog>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>{isSuspended ? t("text_06cdbf20", "إعادة تفعيل المتجر") : t("text_43f05841", "إيقاف المتجر مؤقتًا")}</AlertDialogTitle>
...
const TRANSFER_STATUS: Record<string, { label: string; color: string }> = {
  pending:  { label: t("text_0b836421", "قيد المراجعة"), color: "bg-amber-100 text-amber-700 border-amber-200" },
  approved: { label: t("text_19837e3e", "مقبول"),        color: "bg-green-100 text-green-700 border-green-200" },
  rejected: { label: t("text_20a971a3", "مرفوض"),        color: "bg-red-100 text-red-700 border-red-200" },
};
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 flex-wrap mb-1">
              ...
              <Badge variant="outline" className="text-[10px] px-1.5 py-0 text-muted-foreground">
                {PLAN_LABELS[t.planCode] ?? t.planCode} — {Number(t.amount).toLocaleString("ar-EG")} {t("text_3c111129", t("text_3c111129", "ج.م"))}
                                            </Badge>
            </div>
            <div className="flex items-center gap-3 text-xs text-muted-foreground flex-wrap">
              {t.ownerEmail && <span className="flex items-center gap-1"><Mail className="w-3 h-3" />{t.ownerEmail}</span>}
              <span>{t("text_c2020559", t("text_c2020559", "مرجع:"))} <span className="font-mono font-medium text-foreground">{t.referenceNumber}</span></span>
              <span>{new Date(t.createdAt).toLocaleDateString("ar-EG", { year: "numeric", month: "long", day: "numeric" })}</span>
            </div>
            {t.adminNote && <p className="mt-1 text-xs text-red-600 bg-red-50 px-2 py-1 rounded">{t("text_db8da3e1", t("text_db8da3e1", "ملاحظة:"))} {t.adminNote}</p>}
          </div>
          {t.receiptImageUrl && (
            <a href={t.receiptImageUrl} target="_blank" rel="noopener noreferrer"
              className="shrink-0 flex items-center gap-1 text-xs text-primary hover:underline">
              <Eye className="w-3.5 h-3.5" /> {t("text_227d9c9c", t("text_227d9c9c", "الإيصال"))}
                                      </a>
          )}
        </div>
            ...
            <AlertDialog>
              <AlertDialogTrigger asChild>
                <Button size="sm" className="gap-1.5 bg-green-600 hover:bg-green-700 text-white" disabled={!!loading}>
                  ...
                  {t("text_4fc137cc", t("text_4fc137cc", "قبول وتفعيل"))}
                                                  </Button>
              </AlertDialogTrigger>
              <AlertDialogContent>
                <AlertDialogHeader>
                  <AlertDialogTitle>{t("text_c4e3525c", t("text_c4e3525c", "تأكيد قبول الدفعة"))}</AlertDialogTitle>
                  <AlertDialogDescription>
                    {t("text_a87e4375", t("text_a87e4375", "سيتم تفعيل اشتراك متجر"))} <strong>{t.storeName}</strong> {t("text_b46743e3", t("text_b46743e3", "على خطة"))} <strong>{PLAN_LABELS[t.planCode]}</strong> {t("text_054d420e", t("text_054d420e", "وإنشاء فاتورة مدفوعة تلقائيًا."))}
                                                        </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
                  <AlertDialogCancel>{t("text_b9568e86", t("text_b9568e86", "إلغاء"))}</AlertDialogCancel>
                  <AlertDialogAction onClick={() => act("approve")} className="bg-green-600 hover:bg-green-700">{t("text_d7e02c92", t("text_d7e02c92", "قبول"))}</AlertDialogAction>
                </AlertDialogFooter>
              </AlertDialogContent>
            </AlertDialog>
            ...
            <AlertDialog>
              <AlertDialogTrigger asChild>
                <Button size="sm" variant="outline" className="gap-1.5 text-red-600 border-red-200 hover:bg-red-50" disabled={!!loading}>
                  ...
                  {t("text_b7dee974", t("text_b7dee974", "رفض"))}
                                                  </Button>
              </AlertDialogTrigger>
              <AlertDialogContent>
                <AlertDialogHeader>
                  <AlertDialogTitle>{t("text_4f739f03", t("text_4f739f03", "رفض طلب التحويل"))}</AlertDialogTitle>
                  <AlertDialogDescription>{t("text_c850c90f", t("text_c850c90f", "سيتم إعلام التاجر بالرفض. يمكنك إضافة ملاحظة توضيحية."))}</AlertDialogDescription>
                </AlertDialogHeader>
                <div className="px-6 pb-2">
                  <Textarea
                    placeholder={t("text_1e008745", "سبب الرفض (اختياري)...")}
                    value={rejectNote}
                ...
                <AlertDialogFooter>
                  <AlertDialogCancel>{t("text_b9568e86", t("text_b9568e86", "إلغاء"))}</AlertDialogCancel>
                  <AlertDialogAction onClick={() => act("reject")} className="bg-destructive hover:bg-destructive/90">{t("text_b7dee974", t("text_b7dee974", "رفض"))}</AlertDialogAction>
                </AlertDialogFooter>
              </AlertDialogContent>
            </AlertDialog>
          </div>
...
export default function Platform() {
  ...
  if (authLoading) {
    return <div className="container mx-auto px-4 py-20 text-center text-muted-foreground">{t("text_dc2d2bb8", t("text_dc2d2bb8", "جارٍ التحقق..."))}</div>;
  }
  ...
  if (!merchant?.isPlatformAdmin) {
    return (
      <div className="container mx-auto px-4 py-24 text-center">
        ...
        <p className="text-xl font-bold mb-2">{t("text_ba11b48e", t("text_ba11b48e", "وصول محظور"))}</p>
        <p className="text-muted-foreground mb-6">{t("text_a08ec7d4", t("text_a08ec7d4", "هذه الصفحة متاحة للمشغلين فقط"))}</p>
        <Button asChild><Link href="/dashboard">{t("text_2f091262", t("text_2f091262", "العودة للوحة التحكم"))}</Link></Button>
      </div>
    );
  }
    ...
    <div className="container mx-auto px-4 py-10 max-w-7xl">
      <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }}>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-3xl font-bold">{t("text_2be33c96", t("text_2be33c96", "لوحة المشغّل"))}</h1>
        </div>
        <p className="text-muted-foreground mb-6">{t("text_f09accc0", t("text_f09accc0", "نظرة عامة على جميع المتاجر والنشاط"))}</p>
        <div className="flex gap-2 mb-8 flex-wrap">
          {[
            { id: "merchants", label: t("text_b3a7e634", "المتاجر المسجلة"), icon: Store,       badge: merchants.length || null },
            { id: "payments",  label: t("text_35ffaed9", "المدفوعات"),    icon: CreditCard,  badge: (transferRequests as any[]).filter((t) => t.status === "pending").length || null },
            { id: "tenants",   label: t("text_fb82e920", "دليل المتاجر"),      icon: Users,       badge: null },
            { id: "health",    label: t("text_8cef22c2", "صحة المنصة"),   icon: Heart,       badge: null },
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        {[
          { label: t("text_bc1742ce", "إجمالي المتاجر"),    value: stats?.totalTenants,            icon: Store,         color: "bg-blue-500" },
          { label: t("text_226a9838", "متاجر نشطة"),         value: stats?.activeTenants,           icon: TrendingUp,    color: "bg-green-500" },
          { label: t("text_383a6b80", "متاجر تجريبية"),      value: stats?.trialTenants,            icon: Users,         color: "bg-violet-500" },
          { label: t("text_9a1e99de", "قريبة من الحد"),      value: stats?.tenantsNearProductLimit, icon: AlertTriangle, color: "bg-orange-500" },
        ].map((s) => (
      ...
      {stats && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
          <Card className="border-border/50">
            <CardHeader className="pb-2">
              <CardTitle className="text-sm flex items-center gap-2">
                <BarChart2 className="w-4 h-4 text-primary" /> {t("text_6d161470", t("text_6d161470", "اكتمال الإعداد"))}
                                            </CardTitle>
            </CardHeader>
            <CardContent className="space-y-2">
              {[
                { label: t("text_95159717", "مكتمل"), count: stats.onboardingCompletion.complete,   color: "bg-green-500" },
                { label: t("text_39e5b207", "جزئي"),  count: stats.onboardingCompletion.partial,    color: "bg-yellow-500" },
                { label: t("text_6e50d210", "لم يبدأ"), count: stats.onboardingCompletion.notStarted, color: "bg-red-400" },
              ].map((row) => (
          ...
          <Card className="border-border/50">
            <CardHeader className="pb-2">
              <CardTitle className="text-sm flex items-center gap-2">
                <Package className="w-4 h-4 text-primary" /> {t("text_71cc3d39", t("text_71cc3d39", "توزيع الخطط"))}
                                            </CardTitle>
            </CardHeader>
            <CardContent className="space-y-2">
              {stats.planBreakdown.map((p) => (
                <div key={p.planCode} className="flex items-center gap-3 text-sm">
                  ...
                  <span className="font-bold">{p.count} {t("text_f2e14bab", t("text_f2e14bab", "متجر"))}</span>
                </div>
              ))}
              {stats.planBreakdown.length === 0 && (
                <p className="text-muted-foreground text-xs">{t("text_9dbf18d2", t("text_9dbf18d2", "لا توجد بيانات"))}</p>
              )}
            </CardContent>
          </Card>
        </div>
        ...
        <div>
          <div className="flex flex-col gap-3 mb-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="relative flex-1 max-w-sm">
              ...
              <Input
                placeholder={t("text_e8dd06a4", "بحث بالمتجر أو المالك أو المدينة أو الرابط...")}
                value={search}
            ...
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              ...
              <span>{filteredMerchants.length} {t("text_a4b3f24c", t("text_a4b3f24c", "متجر مسجل"))}</span>
            </div>
          </div>
                          ...
                          <div className="flex items-start gap-3 min-w-0 flex-1">
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              <div className="mt-3 grid grid-cols-3 gap-2 max-w-md">
                                {[
                                  { label: t("text_684b0d48", "منتجات"), value: m.productCount, icon: Package },
                                  { label: t("text_01e1f2bc", "طلبات"), value: m.orderCount, icon: ShoppingBag },
                                  { label: t("text_7fce496f", "إيراد"), value: `${Number(m.totalRevenue ?? 0).toLocaleString("ar-EG")} ج.م`, icon: Banknote },
                                ].map((metric) => (
          ...
          {!merchantsLoading && filteredMerchants.length === 0 && (
            <div className="text-center py-20">
              ...
              <p className="text-muted-foreground">{t("text_cee7eae2", t("text_cee7eae2", "لا توجد متاجر مسجلة تطابق البحث"))}</p>
            </div>
          )}
        </div>
      )}
        ...
        <div className="space-y-8">
          ...
          <div>
            <div className="flex items-center gap-3 mb-4">
              <p className="text-sm font-medium">{t("text_8ac18c7f", t("text_8ac18c7f", "طلبات التحويل البنكي"))}</p>
              <Badge variant="outline" className="text-[10px] text-muted-foreground">
                {(transferRequests as any[]).filter((t) => t.status === "pending").length} {t("text_0b836421", t("text_0b836421", "قيد المراجعة"))}
                                            </Badge>
            </div>
            <motion.div className="space-y-3" variants={stagger.container} initial="hidden" animate="show">
              {transfersLoading
                ...
                : (transferRequests as any[]).length === 0
                  ? <div className="text-center py-20"><CreditCard className="w-10 h-10 text-muted-foreground/30 mx-auto mb-3" /><p className="text-muted-foreground">{t("text_df706c23", t("text_df706c23", "لا توجد طلبات بعد"))}</p></div>
          ...
          <div className="border-t border-border/50 pt-6">
            <div className="flex items-center gap-2 mb-3">
              ...
              <p className="text-sm font-medium">{t("text_dffee086", t("text_dffee086", "تسوية مدفوعات Paymob"))}</p>
              <Badge variant="secondary" className="text-[10px]">reconciliation</Badge>
            </div>
            <Card className="border-border/50">
              <CardContent className="p-4 space-y-3">
                <p className="text-xs text-muted-foreground leading-relaxed">
                  {t("text_25d8aa8a", t("text_25d8aa8a", "يتم تخزين جميع معاملات Paymob في جدول"))} <code className="bg-muted px-1 rounded text-[11px]">payment_records</code>{t("text_23f40f54", ".
                                                    يمكن مراجعة المدفوعات المعلقة أو الفاشلة وتسويتها مع كشف حساب Paymob الشهري.")}
                                                  </p>
                <div className="grid grid-cols-3 gap-3 text-center">
                  {[
                    { label: t("text_03eacf6f", "مكتملة"), color: "text-green-600", key: "captured" },
                    { label: t("text_aacc257a", "معلقة"), color: "text-yellow-600", key: "pending" },
                    { label: t("text_7b770e94", "فاشلة"), color: "text-red-500", key: "failed" },
                  ].map((s) => (
                ...
                <p className="text-[11px] text-muted-foreground">
                  {t("text_23e44662", t("text_23e44662", "للاستعلام المباشر:"))} <code className="bg-muted px-1 rounded">SELECT * FROM payment_records ORDER BY created_at DESC</code>
                </p>
              </CardContent>
            </Card>
          </div>
        ...
        <div>
          <div className="flex items-center gap-3 mb-4">
            <div className="relative flex-1 max-w-sm">
              <Search className="absolute start-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
              <Input
                placeholder={t("text_62ac5ee5", "بحث بالاسم أو الرابط...")}
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                className="ps-10 h-10"
              />
            </div>
            <p className="text-sm text-muted-foreground">{filteredTenants.length} {t("text_f2e14bab", t("text_f2e14bab", "متجر"))}</p>
          </div>
                          ...
                          <div className="flex items-center gap-3 min-w-0">
                            ...
                            <div className="min-w-0">
                              ...
                              <div className="flex items-center gap-2 mt-0.5 text-xs text-muted-foreground">
                                <span>{t("text_d7be69a9", t("text_d7be69a9", "خطة:"))} {PLAN_LABELS[tenant.planCode ?? "starter"] ?? tenant.planCode}</span>
                                <span>·</span>
          ...
          {!tenantsLoading && filteredTenants.length === 0 && (
            <div className="text-center py-20">
              <Store className="w-10 h-10 text-muted-foreground/30 mx-auto mb-3" />
              <p className="text-muted-foreground">{t("text_3bcd91ae", t("text_3bcd91ae", "لا توجد متاجر"))}</p>
            </div>
          )}
        </div>
      )}
        ...
        <motion.div className="space-y-3" variants={stagger.container} initial="hidden" animate="show">
                  ...
                  <Card className="border-border/50">
                    <CardContent className="p-4">
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center text-primary font-bold text-sm shrink-0">
                          {(item.tenantName ?? t("text_494367d0", "؟"))[0]}
                        </div>
          ...
          {!healthLoading && (healthScores as any[]).length === 0 && (
            <div className="text-center py-20">
              ...
              <p className="text-muted-foreground">{t("text_94b0492b", t("text_94b0492b", "لا توجد بيانات صحة بعد"))}</p>
            </div>
          )}
        </motion.div>
      )}
