...
function CodBadge({ rate, totalCod, confirmed, cancelled }: {
  ...
  const config = {
    high:   { label: `${rate}% تأكيد`, cls: "bg-green-100 text-green-800 border-green-200",  Icon: TrendingUp },
    medium: { label: `${rate}% تأكيد`, cls: "bg-amber-100 text-amber-800 border-amber-200",  Icon: Minus },
    low:    { label: `${rate}% تأكيد`, cls: "bg-red-100 text-red-800 border-red-200",        Icon: TrendingDown },
  }[risk];

  return (
    <Badge
      className={`text-[10px] px-1.5 py-0 border shrink-0 flex items-center gap-0.5 ${config.cls}`}
      title={`طلبات COD: ${totalCod} | مؤكدة: ${confirmed} | ملغية: ${cancelled}`}
    >
    ...
    <div className="container mx-auto px-4 py-10">
      <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }}>
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-3xl font-bold">{t("text_8dee682a", t("text_8dee682a", "العملاء"))}</h1>
        </div>
        <p className="text-muted-foreground mb-4">{t("text_e4cd3cfa", t("text_e4cd3cfa", "قاعدة بيانات عملاء المتجر مع معدل تأكيد الدفع عند الاستلام"))}</p>
      </motion.div>

      <GuideCard
        storageKey="customers"
        title={t("text_0ec5f3e7", "فهم قاعدة عملائك")}
        description={t("text_baf87f36", "هنا تجد كل عميل سبق وأجرى طلباً. ابحث عن أي عميل واطّلع على سجل مشترياته ومعدل التأكيد.")}
        tips={[
          t("text_cae2d344", "العملاء ذوو معدل تأكيد منخفض (أقل 50%) — تابع معهم قبل التغليف."),
          t("text_5e907b63", "العملاء المتكررون هم كنزك الحقيقي — أرسل لهم عروضاً خاصة."),
          t("text_7d73917f", "رقم الهاتف المتحقق منه يعني عميلاً أكثر مصداقية لطلبات COD."),
        ]}
        variant="tip"
      />
      ...
      >
        ...
        <Input
          placeholder={t("text_69c8fe38", "بحث باسم العميل أو البريد الإلكتروني...")}
          value={search}
      ...
      {!isLoading && (
        <p className="text-sm text-muted-foreground mb-6">{filtered?.length ?? 0} {t("text_a1d0c2ac", t("text_a1d0c2ac", "عميل"))}</p>
      )}
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {cod.totalOrders > 1 && (
                            <Badge className="text-[10px] px-1.5 py-0 bg-pink-100 text-pink-700 border border-pink-200 shrink-0">
                              <Star className="w-2.5 h-2.5 me-0.5 fill-pink-500 text-pink-500" /> {t("text_b65905b1", t("text_b65905b1", "متكرر"))}
                                                                            </Badge>
                          )}
                          <CodBadge
                        ...
                        {cod.totalOrders !== undefined && (
                          <div className="flex gap-3 mt-2 pt-2 border-t border-border/30 flex-wrap">
                            <span className="text-xs text-muted-foreground flex items-center gap-1">
                              <ShoppingCart className="w-3 h-3" /> {cod.totalOrders} {t("text_f48e3aa7", t("text_f48e3aa7", "طلب"))}
                                                                            </span>
                            {cod.totalSpent > 0 && (
                              <span className="text-xs font-medium text-primary">
                                {Number(cod.totalSpent).toLocaleString("ar-EG")} {t("text_3c111129", t("text_3c111129", "ج.م"))}
                                                                                  </span>
                            )}
                        ...
                        {risk === "low" && (
                          <div className="mt-2 text-[10px] bg-red-50 border border-red-200 text-red-700 rounded-lg px-2 py-1 flex items-center gap-1">
                            ...
                            {t("text_3a1cd1c6", t("text_3a1cd1c6", "خطر عالٍ — معدل إلغاء مرتفع، تأكد قبل الشحن"))}
                                                                      </div>
                        )}
                        {risk === "medium" && (
                          <div className="mt-2 text-[10px] bg-amber-50 border border-amber-200 text-amber-700 rounded-lg px-2 py-1 flex items-center gap-1">
                            ...
                            {t("text_6bdbefe4", t("text_6bdbefe4", "خطر متوسط — اتصل بالعميل قبل الشحن"))}
                                                                      </div>
                        )}
                      </div>
                    </div>
      ...
      {!isLoading && filtered?.length === 0 && (
        <div className="text-center py-24">
          ...
          <p className="text-muted-foreground">{t("text_1a047f46", t("text_1a047f46", "لا يوجد عملاء"))}</p>
        </div>
      )}
    </div>
  );
