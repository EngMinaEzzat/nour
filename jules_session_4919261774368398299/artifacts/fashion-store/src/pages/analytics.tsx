...
const STOCK_STATUS_LABELS: Record<string, { label: string; color: string }> = {
  out_of_stock: { label: t("text_01f4fa1d", "نفد المخزون"), color: "bg-red-100 text-red-700 border-red-200" },
  low_stock:    { label: t("text_ea847c41", "مخزون منخفض"), color: "bg-orange-100 text-orange-700 border-orange-200" },
  ok:           { label: t("text_e73a19f1", "متاح"), color: "bg-green-100 text-green-700 border-green-200" },
};
...
function useAnalytics(tenantId: number | undefined, dateFrom: string, dateTo: string) {
  return useQuery<AnalyticsData>({
    ...
    queryFn: async () => {
      ...
      if (!r.ok) throw new Error(t("text_dfaabe76", "فشل جلب التحليلات"));
      return r.json();
    },
  });
}
  ...
  if (!tenantId) {
    return (
      <div className="container mx-auto px-4 py-12 text-center">
        <p className="text-muted-foreground">{t("text_e814c50a", t("text_e814c50a", "يجب تسجيل الدخول لعرض التحليلات"))}</p>
      </div>
    );
  }
      ...
      <div className="border-b bg-card/60">
        <div className="container mx-auto px-4 py-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <h1 className="text-2xl font-bold">{t("text_fe4c06c2", t("text_fe4c06c2", "التحليلات والأداء"))}</h1>
              <p className="text-muted-foreground text-sm mt-1">{t("text_77f3b4a9", t("text_77f3b4a9", "نظرة شاملة على أداء متجرك"))}</p>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <div className="flex gap-1.5">
                {[
                  { label: t("text_cb3a1ebd", "7 أيام"), days: 7 },
                  { label: t("text_b7ded448", "30 يوم"), days: 30 },
                  { label: t("text_0fecef0b", "90 يوم"), days: 90 },
                ].map((p) => (
      ...
      <div className="container mx-auto px-4 py-8 space-y-8">
        ...
        <motion.div variants={stagger.container} initial="hidden" animate="show" className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {isLoading ? (
            ...
            <>
              <KpiCard title={t("text_f755f8a6", "إجمالي الطلبات")} value={data.totalOrders.toLocaleString("ar-EG")} icon={ShoppingCart} color="bg-blue-100 text-blue-600" />
              <KpiCard title={t("text_d61ce211", "الإيراد الإجمالي")} value={`${data.grossRevenue.toLocaleString("ar-EG")} ج.م`} icon={DollarSign} color="bg-green-100 text-green-600" />
              <KpiCard title={t("text_e0c9aab3", "الإيراد الصافي")} value={`${data.netRevenue.toLocaleString("ar-EG")} ج.م`} sub={t("text_1f9cf831", "مستبعداً الملغي والمُعاد")} icon={TrendingUp} color="bg-emerald-100 text-emerald-600" />
              <KpiCard title={t("text_5bbffe27", "متوسط قيمة الطلب")} value={`${data.avgOrderValue.toLocaleString("ar-EG")} ج.م`} icon={BarChart2} color="bg-violet-100 text-violet-600" />
              <KpiCard title={t("text_a24ff907", "معدل الإلغاء")} value={`${data.cancellationRate}%`} icon={TrendingDown} color="bg-red-100 text-red-600" />
              <KpiCard title={t("text_d19c505f", "معدل الإرجاع")} value={`${data.returnRate}%`} icon={RotateCcw} color="bg-orange-100 text-orange-600" />
              <KpiCard title={t("text_eed3b06c", "عملاء متكررون")} value={data.repeatCustomerCount.toLocaleString("ar-EG")} sub={t("text_9b8e254f", "طلبوا أكثر من مرة")} icon={Users} color="bg-pink-100 text-pink-600" />
              <KpiCard title={t("text_8c895554", "طلبات إرجاع مفتوحة")} value={data.openReturnCases.toLocaleString("ar-EG")} icon={RefreshCcw} color="bg-amber-100 text-amber-600" />
            </>
          ) : null}
        </motion.div>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          ...
          <Card className="md:col-span-2 border-0 shadow-md">
            <CardHeader className="pb-2">
              <CardTitle className="text-base">{t("text_1f17b0f1", t("text_1f17b0f1", "المبيعات اليومية"))}</CardTitle>
            </CardHeader>
            <CardContent>
              {isLoading ? <Skeleton className="h-56 w-full" /> : (
                <ResponsiveContainer width="100%" height={220}>
                  <AreaChart data={data?.salesByDay ?? []}>
                    ...
                    <Tooltip formatter={(v: number) => [`${v.toLocaleString("ar-EG")} ج.م`, t("text_2ce9c33b", "الإيراد")]} />
          ...
          <Card className="border-0 shadow-md">
            <CardHeader className="pb-2">
              <CardTitle className="text-base">{t("text_6e0b0b2f", t("text_6e0b0b2f", "حالات الطلبات"))}</CardTitle>
            </CardHeader>
            <CardContent>
              {isLoading ? <Skeleton className="h-56 w-full" /> : (
                <>
          ...
          <Card className="border-0 shadow-md">
            <CardHeader className="pb-2">
              <CardTitle className="text-base flex items-center gap-2">
                ...
                {t("text_6b612675", t("text_6b612675", "أفضل المنتجات مبيعاً"))}
                                            </CardTitle>
            </CardHeader>
            <CardContent>
              {isLoading ? <Skeleton className="h-40 w-full" /> : (
                <div className="space-y-3">
                  {(data?.topProducts ?? []).slice(0, 5).length === 0 ? (
                    <p className="text-sm text-muted-foreground text-center py-8">{t("text_0ef3cc49", t("text_0ef3cc49", "لا توجد مبيعات في هذه الفترة"))}</p>
                  ) : (
                    (data?.topProducts ?? []).slice(0, 5).map((p, i) => (
                      <div key={p.id} className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-muted-foreground">{p.units_sold} {t("text_ab6cdcaa", t("text_ab6cdcaa", "وحدة"))}</p>
                        </div>
                        <span className="text-sm font-semibold shrink-0">{p.revenue.toLocaleString("ar-EG")} {t("text_3c111129", t("text_3c111129", "ج.م"))}</span>
                      </div>
                    ))
                  )}
                </div>
          ...
          <Card className="border-0 shadow-md">
            <CardHeader className="pb-2">
              <CardTitle className="text-base flex items-center gap-2">
                ...
                {t("text_4b07919d", t("text_4b07919d", "مخزون منخفض / نافد"))}
                                            </CardTitle>
            </CardHeader>
            <CardContent>
              {isLoading ? <Skeleton className="h-40 w-full" /> : (
                <div className="space-y-3">
                  {(data?.lowStockProducts ?? []).length === 0 ? (
                    <p className="text-sm text-muted-foreground text-center py-8">{t("text_1a83f0a5", t("text_1a83f0a5", "لا توجد منتجات بمخزون منخفض 👍"))}</p>
                  ) : (
                    (data?.lowStockProducts ?? []).slice(0, 6).map((p) => {
                      ...
                      return (
                        <div key={p.id} className="flex items-center justify-between gap-2">
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium truncate">{p.name}</p>
                            <p className="text-xs text-muted-foreground">{t("text_a9995669", t("text_a9995669", "متبقٍ:"))} {p.stock} {t("text_99fb92ed", t("text_99fb92ed", "من"))} {p.effective_threshold}</p>
                          </div>
        ...
        {!isLoading && (data?.topProducts ?? []).length > 0 && (
          <Card className="border-0 shadow-md">
            <CardHeader className="pb-2">
              <CardTitle className="text-base">{t("text_8a13c6dc", t("text_8a13c6dc", "مقارنة إيرادات المنتجات"))}</CardTitle>
            </CardHeader>
            <CardContent>
