    ...
    <div className="space-y-4" dir="rtl">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div className="space-y-1.5">
          <Label htmlFor="category-name-ar">{t("text_db2f4d2a", t("text_db2f4d2a", "الاسم العربي"))}</Label>
          <Input
            id="category-name-ar"
            value={form.nameAr}
            onChange={(event) => setForm((current) => ({ ...current, nameAr: event.target.value }))}
            placeholder={t("text_54b4772d", "فساتين")}
          />
        </div>
        <div className="space-y-1.5">
          <Label htmlFor="category-name">{t("text_c90baf69", t("text_c90baf69", "الاسم الداخلي"))}</Label>
          <Input
            id="category-name"
            value={form.name}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div className="space-y-1.5">
          <Label>{t("text_a70b0172", t("text_a70b0172", "النوع"))}</Label>
          <Select
            value={form.type}
          ...
          >
            ...
            <SelectContent>
              <SelectItem value="fashion">{t("text_2f56c4c3", t("text_2f56c4c3", "أزياء"))}</SelectItem>
              <SelectItem value="cosmetics">{t("text_39363b8c", t("text_39363b8c", "تجميل"))}</SelectItem>
            </SelectContent>
          </Select>
        </div>

        <div className="space-y-1.5">
          <Label>{t("text_ccf7fe0b", t("text_ccf7fe0b", "الفئة الأم (اختياري)"))}</Label>
          <Select
            value={form.parentId?.toString() ?? "none"}
            onValueChange={(value) => setForm((current) => ({ ...current, parentId: value === "none" ? null : Number(value) }))}
          >
            <SelectTrigger>
              <SelectValue placeholder={t("text_6fdeffb5", "فئة رئيسية")} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="none">{t("text_8554e18a", t("text_8554e18a", "بدون (فئة رئيسية)"))}</SelectItem>
      ...
      <ImageUpload
        label={t("text_fd5f7098", "صورة الفئة")}
        value={form.imageUrl}
      ...
      <Button className="w-full gap-2" onClick={handleSave} disabled={!canSave}>
        ...
        {category ? t("text_91d6db7f", "حفظ التعديلات") : t("text_21591800", "إضافة الفئة")}
      </Button>
    </div>
  );
}
    ...
    <div className="container mx-auto px-4 py-10">
      ...
      >
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl font-bold">{t("text_95e1474e", t("text_95e1474e", "الفئات"))}</h1>
          </div>
          <p className="text-muted-foreground">{t("text_bb574598", t("text_bb574598", "إدارة فئات المنتجات التي تظهر في المتجر"))}</p>
        </div>
          ...
          <Dialog open={createOpen} onOpenChange={setCreateOpen}>
            <DialogTrigger asChild>
              <Button className="gap-2">
                ...
                {t("text_2052d503", t("text_2052d503", "إضافة فئة"))}
                                            </Button>
            </DialogTrigger>
            <DialogContent>
              <DialogHeader>
                <DialogTitle>{t("text_57cc9dbd", t("text_57cc9dbd", "إضافة فئة جديدة"))}</DialogTitle>
              </DialogHeader>
              ...
              <motion.div key={cat.id} variants={stagger.item}>
                <Card className="border-border/50 overflow-hidden hover:shadow-sm transition-shadow">
                  ...
                  <CardContent className="p-4">
                    ...
                    <p className="text-xs text-muted-foreground mb-4">{cat.productCount} {t("text_bb7632a2", t("text_bb7632a2", "منتج"))}</p>
                    {cat.parentId && (
                      <Badge variant="outline" className="mb-4 text-[10px] font-normal">
                        {t("text_d66f3760", t("text_d66f3760", "فرعية من:"))} {categories.find(p => p.id === cat.parentId)?.nameAr ?? "..."}
                      </Badge>
                    )}
                    <div className="flex items-center gap-2">
                      <Button asChild size="sm" variant="outline" className="flex-1 gap-1.5">
                        <Link href={`/products?categoryId=${cat.id}`}>
                          ...
                          {t("text_b25ceb8b", t("text_b25ceb8b", "المنتجات"))}
                                                                </Link>
                      </Button>
                      {canEdit && (
                        <Button size="sm" variant="outline" className="gap-1.5" onClick={() => setEditing(cat)}>
                          <Edit className="w-3.5 h-3.5" />
                          {t("text_759fdc24", t("text_759fdc24", "تعديل"))}
                                                                </Button>
                      )}
                    </div>
                  </CardContent>
      ...
      {!isLoading && categories?.length === 0 && (
        <div className="text-center py-24">
          ...
          <p className="text-muted-foreground">{t("text_73f6d9f8", t("text_73f6d9f8", "لا توجد فئات بعد"))}</p>
        </div>
      )}
      ...
      <Dialog open={!!editing} onOpenChange={(open) => !open && setEditing(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("text_95f8d4a0", t("text_95f8d4a0", "تعديل الفئة"))}</DialogTitle>
          </DialogHeader>
