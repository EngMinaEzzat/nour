...
const PAYMENT_OPTIONS: { value: PaymentMethod; label: string; desc: string; icon: typeof Banknote }[] = [
  { value: "cod", label: t("text_a4fca794", "الدفع عند الاستلام"), desc: t("text_fae0df03", "ادفع نقداً عند وصول طلبك"), icon: Banknote },
  { value: "paymob", label: t("text_5b6eda84", "الدفع الإلكتروني"), desc: t("text_c332cd5d", "بطاقة ائتمان / فيزا / فوري / محافظ إلكترونية"), icon: CreditCard },
];
...
export default function Checkout() {
  ...
  function validate() {
    ...
    if (!name.trim()) errs.name = t("text_66f77d44", "الاسم مطلوب");
    if (!email.trim() || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) errs.email = t("text_6e20cfe9", "يرجى إدخال بريد إلكتروني صحيح");
    if (!phone.trim()) errs.phone = t("text_d721cc38", "رقم الهاتف مطلوب للتواصل");
    if (!address.trim()) errs.address = t("text_68ec9fcf", "عنوان التوصيل مطلوب");
    setErrors(errs);
  ...
  const applyCoupon = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok && data.valid) {
        ...
        setCoupon((c) => ({ ...c, applying: false, valid: false, error: data.error ?? t("text_de487774", "كود غير صحيح"), discountAmount: 0, codeId: null }));
      }
    } catch {
      setCoupon((c) => ({ ...c, applying: false, valid: false, error: t("text_ca619354", "تعذّر التحقق من الكود") }));
    }
    ...
    try {
      ...
      if (paymentMethod === "paymob" && orderIds.length > 0) {
        ...
        if (initRes.ok && initData.iframeSrc) {
          ...
          setErrors({ submit: initData.error ?? t("text_40a55855", "Paymob غير مُهيأ بعد. تم حفظ طلبك — سيتم الدفع عند التسليم.") });
        }
      }
      ...
      setErrors({ submit: t("text_c118dbc2", "حدث خطأ أثناء تنفيذ الطلب. يرجى المحاولة مرة أخرى.") });
    } finally {
    ...
    return (
      <div className="container mx-auto px-4 py-10 max-w-3xl">
        <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}>
          <h1 className="text-3xl font-bold mb-6 text-center">{t("text_e4a89ce6", t("text_e4a89ce6", "إتمام الدفع الإلكتروني"))}</h1>
          ...
          <p className="text-center text-sm text-muted-foreground mt-4">
            {t("text_8293cd79", t("text_8293cd79", "بعد إتمام الدفع ستصلك رسالة تأكيد على واتساب وبريدك الإلكتروني"))}
                              </p>
          <div className="text-center mt-4">
            <Button variant="outline" className="rounded-full" onClick={() => {
              ...
              {t("text_59b6557f", t("text_59b6557f", "تم الدفع — عرض الطلب"))}
                                    </Button>
          </div>
        </motion.div>
      </div>
  ...
  if (items.length === 0) {
    return (
      <div className="container mx-auto px-4 py-24 text-center">
        ...
        <h1 className="text-3xl font-bold mb-2">{t("text_6e15a378", t("text_6e15a378", "سلة التسوق فارغة"))}</h1>
        <p className="text-muted-foreground mb-8">{t("text_64a7e010", t("text_64a7e010", "أضيفي منتجات قبل الدفع."))}</p>
        <Button asChild className="rounded-full"><Link href="/products">{t("text_1b76bf01", t("text_1b76bf01", "تصفّحي المنتجات"))}</Link></Button>
      </div>
    );
  }
    ...
    <div className="container mx-auto px-4 py-10 pb-24 max-w-5xl">
      <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
        <Button variant="ghost" size="sm" asChild className="mb-6 -ms-2">
          <Link href="/products"><ChevronRight className="w-4 h-4 me-1" /> {t("text_279044fb", t("text_279044fb", "متابعة التسوق"))}</Link>
        </Button>
        <h1 className="text-4xl font-bold text-foreground mb-10">{t("text_f132a196", t("text_f132a196", "الدفع والتوصيل"))}</h1>
      </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-10">
        ...
        <motion.div className="lg:col-span-3 space-y-6"
          ...
          <Card className="border-border/50">
            <CardHeader>
              <CardTitle className="text-xl flex items-center gap-2">
                <User className="w-5 h-5 text-primary" /> {t("text_6fda8ac1", t("text_6fda8ac1", "بيانات العميل"))}
                                            </CardTitle>
            </CardHeader>
            <CardContent className="space-y-5">
              <div className="space-y-1.5">
                <Label htmlFor="name">{t("text_6eacf065", t("text_6eacf065", "الاسم الكامل *"))}</Label>
                <Input id="name" placeholder={t("text_3b71b808", "فاطمة الحسن")} value={name}
              ...
              <div className="space-y-1.5">
                <Label htmlFor="email">
                  <span className="flex items-center gap-1.5"><Mail className="w-3.5 h-3.5" /> {t("text_b052caae", t("text_b052caae", "البريد الإلكتروني *"))}</span>
                </Label>
              ...
              <div className="space-y-1.5">
                <Label htmlFor="phone">
                  <span className="flex items-center gap-1.5"><Phone className="w-3.5 h-3.5" /> {t("text_18f4385f", t("text_18f4385f", "رقم الهاتف *"))} <Badge variant="outline" className="text-xs text-emerald-600 border-emerald-600/30 ms-1">{t("text_d645add1", t("text_d645add1", "للواتساب"))}</Badge></span>
                </Label>
                ...
                <p className="text-xs text-muted-foreground">{t("text_71e1d344", t("text_71e1d344", "سيُستخدم لإرسال تأكيد الطلب على واتساب"))}</p>
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="address">
                  <span className="flex items-center gap-1.5"><MapPin className="w-3.5 h-3.5" /> {t("text_07c4d4ea", t("text_07c4d4ea", "عنوان التوصيل *"))}</span>
                </Label>
                <Input id="address" placeholder={t("text_ca001bee", "١٥ ميدان التحرير، القاهرة")} value={address}
          ...
          <Card className="border-border/50">
            <CardHeader>
              <CardTitle className="text-xl flex items-center gap-2">
                <CreditCard className="w-5 h-5 text-primary" /> {t("text_8f31e70d", t("text_8f31e70d", "طريقة الدفع"))}
                                            </CardTitle>
            </CardHeader>
                ...
                <Card className="border-primary/30 bg-primary/5">
                  <CardContent className="pt-4 pb-4">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="font-semibold text-sm text-primary">{t("text_fe1da80e", t("text_fe1da80e", "الدفع الإلكتروني عبر Paymob"))}</p>
                        <p className="text-xs text-muted-foreground mt-1">
                          {t("text_fcdf2fa5", t("text_fcdf2fa5", "بعد تأكيد الطلب ستُحوَّل إلى صفحة الدفع الآمنة — تدعم فيزا، ماستركارد، فوري، محافظ إلكترونية"))}
                                                                          </p>
                      </div>
                    </div>
                  </CardContent>
        ...
        <motion.div className="lg:col-span-2 space-y-4"
          ...
          <Card className="border-border/50 sticky top-24">
            <CardHeader>
              <CardTitle className="text-xl">{t("text_eeea12fb", t("text_eeea12fb", "ملخص الطلب"))}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
                ...
                <div key={tenantId}>
                  ...
                  {group.items.map((item) => (
                    <div key={`${item.productId}-${item.variantId ?? "base"}`} className="flex items-center gap-3 mb-3">
                      ...
                      <p className="text-sm font-bold shrink-0">{(item.price * item.quantity).toLocaleString("ar-EG")} {t("text_3c111129", t("text_3c111129", "ج.م"))}</p>
                    </div>
                  ))}
              ...
              <div className="space-y-2">
                <p className="text-xs font-semibold text-muted-foreground flex items-center gap-1.5">
                  <Tag className="w-3.5 h-3.5" /> {t("text_c38a00b6", t("text_c38a00b6", "كود الخصم"))}
                                                  </p>
                  ...
                  {coupon.valid ? (
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-green-600">
                          {coupon.type === "percentage" ? `-${coupon.value}%` : coupon.type === "fixed" ? `-${coupon.value.toLocaleString("ar-EG")} ج.م` : t("text_a112716d", "شحن مجاني")}
                        </span>
                      </div>
                    ...
                    <motion.div key="input" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex gap-2">
                      <Input
                        ...
                        placeholder={t("text_ade2a581", "أدخل كود الخصم")}
                      ...
                      <Button
                        ...
                        className="shrink-0"
                      >
                        {coupon.applying ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : t("text_abe3151c", "تطبيق")}
                      </Button>
                    </motion.div>
                  )}
                </AnimatePresence>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between text-sm text-muted-foreground">
                  <span>{t("text_d89e6acb", t("text_d89e6acb", "المجموع الفرعي"))}</span>
                  <span>{totalPrice.toLocaleString("ar-EG")} {t("text_3c111129", t("text_3c111129", "ج.م"))}</span>
                </div>
                {coupon.valid && coupon.discountAmount > 0 && (
                  ...
                  >
                    <span>{t("text_48b7766d", t("text_48b7766d", "الخصم"))}</span>
                    <span>−{coupon.discountAmount.toLocaleString("ar-EG")} {t("text_3c111129", t("text_3c111129", "ج.م"))}</span>
                  </motion.div>
                )}
                <div className="flex items-center justify-between pt-1">
                  <span className="font-semibold text-foreground">{t("text_88fc73eb", t("text_88fc73eb", "الإجمالي"))}</span>
                  <span className="text-2xl font-bold text-primary">{finalTotal.toLocaleString("ar-EG")} {t("text_3c111129", t("text_3c111129", "ج.م"))}</span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 py-2 px-3 bg-muted/50 rounded-xl text-xs text-muted-foreground">
                ...
                <span>{paymentMethod === "paymob" ? t("text_6deb1a19", "دفع إلكتروني عبر Paymob") : t("text_c23f1e7d", "دفع عند الاستلام")}</span>
              </div>
              ...
              <Button className="w-full h-13 text-base rounded-2xl mt-1" onClick={handlePlaceOrder} disabled={isSubmitting}>
                {isSubmitting ? (
                  <><Loader2 className="w-4 h-4 me-2 animate-spin" /> {t("text_84f3f909", t("text_84f3f909", "جارٍ تنفيذ الطلب..."))}</>
                ) : paymentMethod === "paymob" ? (
                  <><CreditCard className="w-4 h-4 me-2" /> {t("text_a39c4d19", t("text_a39c4d19", "تأكيد والانتقال للدفع"))}</>
                ) : (
                  <><CheckCircle2 className="w-4 h-4 me-2" /> {t("text_438227b1", t("text_438227b1", "تأكيد الطلب"))}</>
                )}
              </Button>
              <p className="text-xs text-center text-muted-foreground">{t("text_c3ea0a99", t("text_c3ea0a99", "بالطلب توافقين على شروط الخدمة"))}</p>
            </CardContent>
          </Card>
        </motion.div>
      </div>
