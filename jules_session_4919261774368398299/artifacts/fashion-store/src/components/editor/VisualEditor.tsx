...
export default function VisualEditor({
  ...
  async function handleSave() {
    ...
    try {
      ...
      toast({ title: t("text_201aed2e", "تم الحفظ ✓"), description: t("text_8cda100e", "تم حفظ جميع التغييرات بنجاح") });
    } catch {
      toast({ title: t("text_2b87bca4", "خطأ في الحفظ"), description: t("text_ba2d6050", "حاول مرة أخرى"), variant: "destructive" });
    } finally {
      setSaving(false);
    }
  }
        ...
        <div className="flex-1 flex flex-col overflow-hidden relative">
          <div className="lg:hidden bg-white border-b border-stone-200 px-3 py-2 flex items-center justify-between gap-2" dir="rtl">
            <button
              ...
              aria-label={t("text_9866de7f", "فتح أقسام المتجر")}
              aria-expanded={sectionsOpen}
            >
              <Menu className="w-4 h-4" />
              {t("text_9649d11a", t("text_9649d11a", "أقسام المتجر"))}
                                      </button>
            <div className="flex min-w-0 items-center gap-2 text-xs text-stone-500">
              <Layers3 className="w-4 h-4 shrink-0 text-[#8B1A35]" />
              <span className="truncate">
                {selectedSection ? `تعدلين: ${selectedSection.label}` : t("text_a2b6a25e", "اختاري جزء من المتجر للتعديل")}
              </span>
            </div>
          </div>
          ...
          {sectionsOpen && (
            <>
              ...
              <motion.aside
                ...
                dir="rtl"
                aria-label={t("text_976f47be", "قائمة أقسام المتجر")}
              >
                <div className="flex items-center justify-between border-b border-stone-100 p-4">
                  <div>
                    <p className="text-sm font-semibold text-stone-900">{t("text_b70d5679", t("text_b70d5679", "اختاري جزء من القصة"))}</p>
                    <p className="mt-1 text-xs text-stone-500">{t("text_63f4cc2b", t("text_63f4cc2b", "اضغطي على القسم، هنقفل القائمة ونفتح تعديلاته مباشرة."))}</p>
                  </div>
                  <button
                    onClick={() => setSectionsOpen(false)}
                    className="flex h-10 w-10 items-center justify-center rounded-xl bg-stone-100 text-stone-600"
                    aria-label={t("text_cd3466b4", "إغلاق قائمة الأقسام")}
                  >
                    <X className="h-4 w-4" />
                  </button>
                </div>
