...
async function uploadImage(file: File) {
  ...
  if (!res.ok || !data.url) throw new Error(data.error ?? t("text_4d577c7f", "فشل رفع الصورة"));
  return data.url;
}
...
async function normalizeUploadFile(file: File) {
  ...
  try {
    ...
    if (!ctx) throw new Error(t("text_a40119b6", "تعذر تجهيز الصورة للرفع"));
    ctx.drawImage(image, 0, 0);

    const blob = await new Promise<Blob | null>((resolve) => canvas.toBlob(resolve, "image/jpeg", 0.9));
    if (!blob) throw new Error(t("text_22281c6a", "تعذر تحويل الصورة لصيغة مناسبة للموبايل"));
...
export function ImageUpload({ value, onChange, label, className }: ImageUploadProps) {
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : t("text_4d577c7f", "فشل رفع الصورة"));
    } finally {
      setUploading(false);
    }
  }
    ...
    <div className={className}>
      ...
      <div className="flex gap-2 mb-2">
        <Button
          ...
          onClick={() => setMode("url")}
        >
          <Link2 className="w-3 h-3" /> {t("text_faa0a5c5", t("text_faa0a5c5", "رابط URL"))}
                          </Button>
        <Button
          type="button"
          size="sm"
        ...
        >
          <Upload className="w-3 h-3" /> {t("text_d31448ad", t("text_d31448ad", "رفع صورة"))}
                          </Button>
      </div>
        ...
        >
          ...
          {uploading ? (
            <div className="py-3">
              ...
              <p className="text-xs text-muted-foreground">{t("text_cddf9fd8", t("text_cddf9fd8", "جاري رفع الصورة..."))}</p>
            </div>
          ) : (
            <div className="py-3">
              <Upload className="w-7 h-7 text-muted-foreground/60 mx-auto mb-2" />
              <p className="text-sm font-medium text-foreground">{t("text_2890a9cd", t("text_2890a9cd", "اضغط أو اسحب صورة هنا"))}</p>
              <p className="text-xs text-muted-foreground mt-1">{SUPPORTED_IMAGE_FORMATS} {t("text_9a008ff6", t("text_9a008ff6", "حتى"))} {MAX_IMAGE_SIZE_MB}{t("text_c1cd07d5", t("text_c1cd07d5", "MB بدون ضغط"))}</p>
            </div>
          )}
        </label>
      )}
...
export function ImageUploadList({ values, onChange, label, className }: ImageUploadListProps) {
  ...
  async function handleFiles(fileList: FileList | File[]) {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : t("text_f1d47db7", "فشل رفع الصور"));
    } finally {
      setUploading(false);
    }
  }
      ...
      >
        <div className="flex items-center gap-2">
          ...
          >
            ...
            {uploading ? t("text_fd3ab6d3", "جاري الرفع...") : t("text_79192cd6", "رفع صور")}
          </Button>
          <label htmlFor={inputId} className="text-xs text-muted-foreground cursor-pointer">
            {SUPPORTED_IMAGE_FORMATS} {t("text_9a008ff6", t("text_9a008ff6", "حتى"))} {MAX_IMAGE_SIZE_MB}{t("text_c9c6c940", t("text_c9c6c940", "MB للصورة"))}
                                </label>
        </div>
        <div className="mt-2 flex flex-col gap-2 sm:flex-row">
          ...
          >
            ...
            {t("text_53750875", t("text_53750875", "إضافة رابط"))}
                                </Button>
        </div>
        <input
          id={inputId}
