...
- Vercel runtime still needs correct environment variables before signup testing: `DATABASE_URL` must point to the staging/test Supabase database and `SESSION_SECRET` must be set in the Vercel environment being deployed. Current Vercel CLI output shows these are set for Preview, but not for Production.
