...
const SECTION_RELATED_ACTIONS: Partial<
  Record<SectionConfig["type"], { href: string; label: string; desc: string }>
> = {
  hero: {
...
export default function InspectorPanel({
  ...
  if (!section) {
    ...
    return (
      <div
        className="w-72 bg-white border-r border-stone-200 flex flex-col items-center justify-center p-8 text-center"
        dir={i18n.dir()}
      >
        ...
        <p className="text-sm font-medium text-stone-700 mb-1">
          {t("inspectorPanel.empty.title")}
        </p>
        <p className="text-xs text-stone-400">
          {t("inspectorPanel.empty.desc")}
        </p>
      </div>
    );
  }
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-stone-800">
              {t(`sections.${section.type}`, {
                defaultValue: SECTION_LABELS[section.type],
              })}
            </span>
          </div>
          <div className="flex items-center gap-1">
            ...
            <button
              ...
              title={
                section.visible
                  ? t("inspectorPanel.buttons.hide")
                  : t("inspectorPanel.buttons.show")
              }
            >
              {section.visible ? (
  ...
  return (
    <div className="rounded-xl border border-[#8B1A35]/15 bg-[#8B1A35]/5 p-3">
      <p className="text-xs font-semibold text-stone-800">
        {t("inspectorPanel.actions.title")}
      </p>
...
function ItemListEditor({
  ...
  fields: Array<{
    key: string;
    label: string;
    type: "text" | "textarea" | "image" | "category-select";
  }>;
  categories?: CategoryOption[];
  ...
  return (
    <div className="space-y-3 mt-4 border-t border-stone-100 pt-4">
      ...
      {items.map((item, i) => (
        <div
          key={i}
          className="border border-stone-200 rounded-lg p-3 space-y-3 relative bg-stone-50/50"
        >
          <button
            ...
            {fields.map((f) => (
              <div key={f.key}>
                ...
                {f.type === "textarea" ? (
                  ...
                  >
                    <option value="">
                      {t("inspectorPanel.fields.allProducts", "All Products")}
                    </option>
                    {categories.map((cat) => (
...
function SectionFields({
  ...
  switch (section.type) {
        ...
        <>
          ...
          <Field label={t("inspectorPanel.fields.subheading")}>
            <Input
              ...
              placeholder={defaultSection.content.subheading}
              onChange={(e) => patchContent({ subheading: e.target.value })}
          ...
          <Field label={t("inspectorPanel.fields.ctaText")}>
            <Input
              ...
              placeholder={defaultSection.content.ctaText}
          ...
          <SelectField
            ...
            options={[
              { value: "short", label: t("inspectorPanel.fields.heightShort") },
              {
                value: "medium",
                label: t("inspectorPanel.fields.heightMedium"),
              },
              { value: "tall", label: t("inspectorPanel.fields.heightTall") },
              { value: "full", label: t("inspectorPanel.fields.heightFull") },
            ]}
            onChange={(v) =>
              patchSettings({
                height: v as SectionConfig["settings"]["height"],
              })
            }
          />
          <SelectField
            ...
            options={[
              {
                value: "right",
                label: t("inspectorPanel.fields.textAlignRight"),
              },
              {
                value: "center",
                label: t("inspectorPanel.fields.textAlignCenter"),
              },
              {
                value: "left",
                label: t("inspectorPanel.fields.textAlignLeft"),
              },
            ]}
            onChange={(v) =>
              patchSettings({ textAlign: v as "left" | "center" | "right" })
            }
          />
          <SliderField
            ...
            min={0}
            max={80}
            value={section.settings.overlayOpacity ?? 40}
            onChange={(v) => patchSettings({ overlayOpacity: v })}
            unit="%"
          />
          <Field
            label={t("inspectorPanel.fields.imageUrl")}
            hint={t("inspectorPanel.fields.imageUrlHint")}
          >
            <Input
      ...
      return (
        <>
          ...
          <SelectField
            ...
            options={[
              { value: "grid", label: t("inspectorPanel.fields.layoutGrid") },
              {
                value: "carousel",
                label: t("inspectorPanel.fields.layoutCarousel"),
              },
            ]}
            onChange={(v) =>
              patchSettings({ cardStyle: v as "grid" | "carousel" })
            }
          />
          <ToggleField
            label={t("inspectorPanel.fields.showPrices")}
            value={section.settings.showPrices ?? true}
            onChange={(v) => patchSettings({ showPrices: v })}
          />
          <ToggleField
            label={t("inspectorPanel.fields.showQuickAdd")}
            value={section.settings.showQuickAdd ?? true}
            onChange={(v) => patchSettings({ showQuickAdd: v })}
          />
        </>
      );
        ...
        <>
          ...
          <SelectField
            ...
            options={[
              ...
              {
                value: "editorial",
                label: t("inspectorPanel.fields.layoutEditorial"),
              },
            ]}
            onChange={(v) =>
              patchSettings({ layout: v as "grid" | "carousel" | "editorial" })
            }
          />
        </>
      );
      ...
      return (
        <>
          ...
          <SelectField
            ...
            onChange={(v) =>
              patchSettings({ layout: v as "grid" | "carousel" })
            }
          />
          <ToggleField
            label={t("inspectorPanel.fields.showRating")}
            value={section.settings.showRating ?? true}
            onChange={(v) => patchSettings({ showRating: v })}
          />
          <ItemListEditor
            ...
            fields={[
              {
                key: "name",
                label: t("inspectorPanel.fields.testimonialName"),
                type: "text",
              },
              {
                key: "text",
                label: t("inspectorPanel.fields.testimonialText"),
                type: "textarea",
              },
              {
                key: "rating",
                label: t("inspectorPanel.fields.testimonialRating"),
                type: "text",
              },
            ]}
      ...
      return (
        <div className="space-y-4">
          <div className="pb-4 border-b border-border/50 space-y-3">
            <div className="flex items-center justify-between mb-2">
              <h4 className="text-xs font-semibold text-muted-foreground">
                {t("inspectorPanel.fields.promo1Title")}
              </h4>
              <ToggleField
                label=""
                value={showPromo1 as boolean}
                onChange={(v) => patchSettings({ showPromo1: v })}
              />
            </div>
            {showPromo1 && (
              <>
                <Field label={t("inspectorPanel.fields.promo1Label")}>
                  <Input
                    value={(section.content.promo1Label as string) ?? ""}
                    placeholder={defaultSection.content.promo1Label as string}
                    onChange={(e) =>
                      patchContent({ promo1Label: e.target.value })
                    }
                    className={`text-xs text-start`}
                  />
                </Field>
                <Field label={t("inspectorPanel.fields.promo1Heading")}>
                  <Input
                    value={(section.content.promo1Heading as string) ?? ""}
                    placeholder={defaultSection.content.promo1Heading as string}
                    onChange={(e) =>
                      patchContent({ promo1Heading: e.target.value })
                    }
                    className={`text-xs text-start`}
                  />
                </Field>
                <Field label={t("inspectorPanel.fields.promo1Discount")}>
                  <Input
                    type="number"
                    value={(section.content.promo1Discount as string) ?? ""}
                    placeholder={
                      defaultSection.content.promo1Discount as string
                    }
                    onChange={(e) =>
                      patchContent({ promo1Discount: e.target.value })
                    }
                    className={`text-xs text-start`}
                  />
                </Field>
                <Field label={t("inspectorPanel.fields.promo1Desc")}>
                  <Input
                    value={(section.content.promo1Desc as string) ?? ""}
                    placeholder={defaultSection.content.promo1Desc as string}
                    onChange={(e) =>
                      patchContent({ promo1Desc: e.target.value })
                    }
                    className={`text-xs text-start`}
                  />
                </Field>
                <Field label={t("inspectorPanel.fields.promo1Cta")}>
                  <Input
                    value={(section.content.promo1Cta as string) ?? ""}
                    placeholder={defaultSection.content.promo1Cta as string}
                    onChange={(e) =>
                      patchContent({ promo1Cta: e.target.value })
                    }
                    className={`text-xs text-start`}
                  />
                </Field>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between mb-2">
              <h4 className="text-xs font-semibold text-muted-foreground">
                {t("inspectorPanel.fields.promo2Title")}
              </h4>
              <ToggleField
                label=""
                value={showPromo2 as boolean}
                onChange={(v) => patchSettings({ showPromo2: v })}
              />
            </div>
            {showPromo2 && (
              <>
                <Field label={t("inspectorPanel.fields.promo2Label")}>
                  <Input
                    value={(section.content.promo2Label as string) ?? ""}
                    placeholder={defaultSection.content.promo2Label as string}
                    onChange={(e) =>
                      patchContent({ promo2Label: e.target.value })
                    }
                    className={`text-xs text-start`}
                  />
                </Field>
                <Field label={t("inspectorPanel.fields.promo2Heading")}>
                  <Input
                    value={(section.content.promo2Heading as string) ?? ""}
                    placeholder={defaultSection.content.promo2Heading as string}
                    onChange={(e) =>
                      patchContent({ promo2Heading: e.target.value })
                    }
                    className={`text-xs text-start`}
                  />
                </Field>
                <Field label={t("inspectorPanel.fields.promo2Subheading")}>
                  <Input
                    value={(section.content.promo2Subheading as string) ?? ""}
                    placeholder={
                      defaultSection.content.promo2Subheading as string
                    }
                    onChange={(e) =>
                      patchContent({ promo2Subheading: e.target.value })
                    }
                    className={`text-xs text-start`}
                  />
                </Field>
                <Field label={t("inspectorPanel.fields.promo2Threshold")}>
                  <Input
                    type="number"
                    value={(section.content.promo2Threshold as string) ?? ""}
                    placeholder={
                      defaultSection.content.promo2Threshold as string
                    }
                    onChange={(e) =>
                      patchContent({ promo2Threshold: e.target.value })
                    }
                    className={`text-xs text-start`}
                  />
                </Field>
                <Field label={t("inspectorPanel.fields.promo2Cta")}>
                  <Input
                    value={(section.content.promo2Cta as string) ?? ""}
                    placeholder={defaultSection.content.promo2Cta as string}
                    onChange={(e) =>
                      patchContent({ promo2Cta: e.target.value })
                    }
                    className={`text-xs text-start`}
                  />
                </Field>
        ...
        <>
          ...
          <SelectField
            ...
            options={[
              {
                value: "with-image",
                label: t("inspectorPanel.fields.layoutWithImage"),
              },
              {
                value: "text-only",
                label: t("inspectorPanel.fields.layoutTextOnly"),
              },
            ]}
            onChange={(v) =>
              patchSettings({ layout: v as "with-image" | "text-only" })
            }
          />
        </>
      );
        ...
        <>
          ...
          <ItemListEditor
            ...
            fields={[
              {
                key: "q",
                label: t("inspectorPanel.fields.faqQ"),
                type: "text",
              },
              {
                key: "a",
                label: t("inspectorPanel.fields.faqA"),
                type: "textarea",
              },
            ]}
            onChange={(items) => patchContent({ items })}
    ...
    case "whatsapp":
      return (
        <>
          ...
          <ToggleField
            label={t("inspectorPanel.fields.floatingButton")}
            value={section.settings.floatingButton ?? true}
            onChange={(v) => patchSettings({ floatingButton: v })}
          />
        </>
      );
        ...
        <>
            ...
            fields={
              section.type === "lookbook"
                ? [
                    ...
                    {
                      key: "title",
                      label: t("inspectorPanel.fields.title"),
                      type: "text",
                    },
                    {
                      key: "desc",
                      label: t("inspectorPanel.fields.desc"),
                      type: "text",
                    },
                    {
                      key: "tag",
                      label: t("inspectorPanel.fields.tag"),
                      type: "text",
                    },
                    {
                      key: "categoryId",
                      label: t("inspectorPanel.fields.categoryLink"),
          ...
          {section.type === "lookbook" && (
            <p className="text-[11px] text-stone-500 mt-2 bg-stone-50 border border-stone-150 p-2.5 rounded-lg leading-relaxed">
              {t(
                "inspectorPanel.fields.lookbookHint",
                "💡 ربط الصورة بقسم يتيح للعملاء تصفح منتجات هذا القسم مباشرة عند النقر على الصورة في المتجر. استخدمي \\n في العنوان لسطر جديد.",
              )}
            </p>
          )}
        </>
        ...
        <>
          <ItemListEditor
            ...
            fields={[
              {
                key: "icon",
                label: t("inspectorPanel.fields.trustStripIcon"),
                type: "text",
              },
              {
                key: "title",
                label: t("inspectorPanel.fields.trustStripTitle"),
                type: "text",
              },
              {
                key: "text",
                label: t("inspectorPanel.fields.trustStripText"),
                type: "text",
              },
            ]}
            onChange={(items) => patchContent({ items })}
    ...
    default:
      return (
        <p className="text-xs text-stone-400">
          {t("inspectorPanel.fields.noSettings")}
        </p>
      );
  }
}
