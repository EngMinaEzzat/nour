...
export default function EditorLeftSidebar({
  ...
  function dismissSidebarHint() {
    ...
    try {
      localStorage.setItem("nour_sidebar_hint_seen", "1");
    } catch {}
  }

  const sections = [...config.homepage.sections].sort(
    (a, b) => a.order - b.order,
  );
  ...
  function moveSection(id: string, dir: "up" | "down") {
    const sorted = [...config.homepage.sections].sort(
      (a, b) => a.order - b.order,
    );
    const idx = sorted.findIndex((s) => s.id === id);
    if (
      (dir === "up" && idx === 0) ||
      (dir === "down" && idx === sorted.length - 1)
    )
      return;
  ...
  function addSection(type: SectionType) {
    ...
    onConfigChange({
      ...config,
      homepage: { sections: [...config.homepage.sections, s] },
    });
  ...
  const TABS = [
    {
      key: "sections" as SidebarTab,
      icon: LayoutList,
      label: t("editorSidebar.tabs.sections"),
    },
    {
      key: "theme" as SidebarTab,
      icon: Palette,
      label: t("editorSidebar.tabs.theme"),
    },
  ...
  return (
    <div
      className={`w-64 bg-white border-l border-stone-200 flex flex-col h-full ${className}`}
      dir={i18n.dir()}
    >
      {/* Tabs */}
                ...
                <div className="flex gap-2">
                  ...
                  <div>
                    <p className="text-xs font-bold text-rose-900">
                      {t("storeStoryGuide.title", "Your store story")}
                    </p>
                    <p
                      className={`text-[11px] text-rose-700 leading-relaxed mt-0.5`}
                    >
                      {t(
                        "storeStoryGuide.intro",
                        "I'll help you shape what customers see before they order.",
                      )}
                    </p>
                  </div>
                </div>
            ...
            <div className="p-2 space-y-4 mt-2">
              {STORY_CHAPTERS.map((chapter) => {
                const chapterSections = sections.filter(
                  (s) => getChapterForSection(s.type)?.id === chapter.id,
                );
                        ...
                        <div className="flex gap-0.5">
                          {Array.from({ length: progress.total }).map(
                            (_, i) => (
                              <div
                                key={i}
                                className={`w-1.5 h-1.5 rounded-full ${i < progress.done ? "bg-green-500" : "bg-stone-200"}`}
                              />
                            ),
                          )}
                        </div>
                          ...
                          >
                            <div className="flex items-center gap-2.5">
                              ...
                              <div className="flex-1 min-w-0">
                                <p
                                  className={`text-xs font-medium truncate ${section.visible ? "text-stone-800" : "text-stone-400"}`}
                                >
                                  {t(`sections.${section.type}`)}
                                </p>
                                {!section.visible && (
                                  <p className="text-[10px] text-stone-400">
                                    {t("editorSidebar.sectionItem.hidden")}
                                  </p>
                                )}
                              </div>

                              <div
                                className={`flex items-center gap-0.5 ${isSelected ? "opacity-100" : "opacity-0 group-hover:opacity-100"} transition-opacity`}
                              >
                                <button
                                  onClick={(e) => {
                                    e.stopPropagation();
            ...
            >
              <div className="p-4 border-b border-stone-100">
                <p className="font-medium text-stone-800">
                  {gender === "female"
                    ? t("editorSidebar.addSection.titleFemale")
                    : t("editorSidebar.addSection.titleMale")}
                </p>
                <p className="text-xs text-stone-400 mt-0.5">
                  {gender === "female"
                    ? t("editorSidebar.addSection.descFemale")
                    : t("editorSidebar.addSection.descMale")}
                </p>
              </div>
...
const getVisualThemes = (t: any) => [
  ...
  },
  {
    ...
    name: t("editorSidebar.themePanel.themes.streetWear.name") || t("editorSidebar.themePanel.presets.streetWear.name"),
    desc: t("editorSidebar.themePanel.themes.streetWear.desc") || t("editorSidebar.themePanel.presets.streetWear.desc"),
    emoji: "⚡",
    gradient: "from-[#0f0f0f] to-[#39ff14]",
    preview: { bg: "#f8f9fa", accent: "#39ff14", text: "#0f0f0f" },
    theme: {
      primaryColor: "#39ff14",
  ...
  {
    id: "boho-chic",
    name: t("editorSidebar.themePanel.themes.bohoChic.name") || "بوهيمي عضوي",
    desc: t("editorSidebar.themePanel.themes.bohoChic.desc") || "جمال طبيعي مستوحى من الأرض",
    emoji: "🌿",
    gradient: "from-[#e2725b] to-[#d4a373]",
    preview: { bg: "#faf8f5", accent: "#e2725b", text: "#5c4d43" },
    theme: {
      primaryColor: "#e2725b",
  ...
  {
    id: "dynamic-active",
    name: t("editorSidebar.themePanel.themes.dynamicActive.name") || "طاقة وحركة",
    desc: t("editorSidebar.themePanel.themes.dynamicActive.desc") || "أداء لا يتوقف",
    emoji: "🔥",
    gradient: "from-[#ff4500] to-[#2b2b2b]",
    preview: { bg: "#f5f5f5", accent: "#ff4500", text: "#1a1a1a" },
    theme: {
      primaryColor: "#ff4500",
  ...
  {
    id: "dark-glamour",
    name: t("editorSidebar.themePanel.themes.darkGlamour.name") || "فخامة داكنة",
    desc: t("editorSidebar.themePanel.themes.darkGlamour.desc") || "ألوان غامضة ولمسات ذهبية",
    emoji: "🖤",
    gradient: "from-[#1a0000] to-[#d4af37]",
    preview: { bg: "#111111", accent: "#d4af37", text: "#ffffff" },
    theme: {
      primaryColor: "#d4af37",
  ...
  },
  {
    ...
    name: t("editorSidebar.themePanel.themes.avantGarde.name") || "أفانت جارد",
    desc: t("editorSidebar.themePanel.themes.avantGarde.desc") || "جرأة وتفرد",
    emoji: "⬛",
    gradient: "from-[#000000] to-[#0014ff]",
  ...
  {
    id: "dreamy-pastel",
    name: t("editorSidebar.themePanel.themes.dreamyPastel.name") || "حالم وناعم",
    desc: t("editorSidebar.themePanel.themes.dreamyPastel.desc") || "ألوان الباستيل الهادئة",
    emoji: "☁️",
    gradient: "from-[#ffb6c1] to-[#e6e6fa]",
    preview: { bg: "#fffafb", accent: "#ffb6c1", text: "#4a4a4a" },
    theme: {
      primaryColor: "#ffb6c1",
  ...
  {
    id: "cloud-glow",
    name: t("editorSidebar.themePanel.themes.cloudGlow.name") || "إشراقة السحاب",
    desc: t("editorSidebar.themePanel.themes.cloudGlow.desc") || "بشرة زجاجية",
    emoji: "💧",
    gradient: "from-[#87ceeb] to-[#ffe4e1]",
  ...
  {
    id: "ethereal-minimal",
    name: t("editorSidebar.themePanel.themes.etherealMinimal.name") || "ملائكي نقي",
    desc: t("editorSidebar.themePanel.themes.etherealMinimal.desc") || "بساطة فائقة",
    emoji: "🕊️",
    gradient: "from-[#dcdcdc] to-[#f0f8ff]",
    preview: { bg: "#fdfdfd", accent: "#dcdcdc", text: "#111111" },
    theme: {
      primaryColor: "#dcdcdc",
  ...
  {
    id: "royal-velvet",
    name: t("editorSidebar.themePanel.themes.royalVelvet.name") || "مخمل ملكي",
    desc: t("editorSidebar.themePanel.themes.royalVelvet.desc") || "فخامة عميقة",
    emoji: "👑",
    gradient: "from-[#d4af37] to-[#013220]",
    preview: { bg: "#0a0a0a", accent: "#d4af37", text: "#ffffff" },
    theme: {
      primaryColor: "#d4af37",
  ...
  {
    id: "golden-opulence",
    name: t("editorSidebar.themePanel.themes.goldenOpulence.name") || "رفاهية ذهبية",
    desc: t("editorSidebar.themePanel.themes.goldenOpulence.desc") || "فخامة لا تضاهى",
    emoji: "✨",
    gradient: "from-[#ffd700] to-[#ffffff]",
    preview: { bg: "#fafafa", accent: "#ffd700", text: "#111111" },
    theme: {
      primaryColor: "#ffd700",
  ...
  {
    id: "imperial-chic",
    name: t("editorSidebar.themePanel.themes.imperialChic.name") || "إمبراطورية فاخرة",
    desc: t("editorSidebar.themePanel.themes.imperialChic.desc") || "تصميم ملكي مستوحى من القصور",
    emoji: "🏛️",
    gradient: "from-[#800020] to-[#cfb53b]",
    preview: { bg: "#fdfbf7", accent: "#800020", text: "#1a0a0e" },
    theme: {
      primaryColor: "#800020",
...
function ThemePanel({
  ...
  config: StoreConfig;
  onConfigChange: (c: StoreConfig) => void;
}) {
  const { t, i18n } = useTranslation();
    ...
    <div className="flex-1 overflow-y-auto" dir={i18n.dir()}>
      ...
      <div className="p-3">
        <p className="text-xs font-semibold text-stone-500 mb-2 uppercase tracking-wide">
          {t("editorSidebar.themePanel.title")}
        </p>
        <div className="grid grid-cols-2 gap-2">
          {getVisualThemes(t).map((tTheme) => {
            const isActive =
              activeThemeId === tTheme.id ||
              (!activeThemeId &&
              ...
              >
                ...
                >
                  ...
                  <div
                    ...
                    style={{
                      background: `linear-gradient(135deg, ${tTheme.preview.accent}22, ${tTheme.preview.accent}44)`,
                    }}
                  />
                  <div
                  ...
                  <div
                    className={`absolute top-2 right-2 w-full h-px bg-gradient-to-r ${tTheme.gradient} opacity-60`}
                  />
                  {isActive && (
                    <motion.div
                      initial={{ scale: 0 }}
      ...
      <div className="border-t border-stone-100">
        ...
        >
          <span className="font-medium">
            {t("editorSidebar.themePanel.customOptions")}
          </span>
          <motion.span
            ...
            >
              <div>
                <p className="text-xs font-medium text-stone-500 mb-2">
                  {t("editorSidebar.themePanel.primaryColor")}
                </p>
                <div className="flex flex-wrap gap-1.5">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      patchTheme({ primaryColor: e.target.value })
                    }
                ...
                >
                  <div className="flex items-center gap-1.5 font-medium">
                    ...
                    {primaryContrast.level === "pass"
                      ? t(
                          "editorSidebar.themePanel.contrast.pass",
                          "Readable CTA contrast",
                        )
                      : t(
                          "editorSidebar.themePanel.contrast.risk",
                          "Contrast needs review before publishing",
                        )}
                  </div>
                  <p className="mt-1 text-[10px] opacity-80">
                    {t("editorSidebar.themePanel.contrast.ratio", {
                      ratio: primaryContrast.ratio.toFixed(1),
                      defaultValue: "Contrast ratio {{ratio}}:1",
                    })}
                  </p>
                </div>
              </div>
              ...
              <div>
                <p className="text-xs font-medium text-stone-500 mb-2">
                  {t(
                    "editorSidebar.themePanel.secondaryColor",
                    "Secondary Color",
                  )}
                </p>
                <div className="flex flex-wrap gap-1.5">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      patchTheme({ secondaryColor: e.target.value })
                    }
                    className="w-6 h-6 rounded-full cursor-pointer border-0"
                  />
                </div>
              ...
              <div>
                <p className="text-xs font-medium text-stone-500 mb-2">
                  {t("editorSidebar.themePanel.fontPairing", "Font Pairing")}
                </p>
                <div className="flex flex-col gap-1.5">
                  {(["sans-sans", "serif-sans", "serif-serif"] as const).map(
                    (f) => (
                      <button
                        key={f}
                      ...
                      >
                        {f === "sans-sans"
                          ? t(
                              "editorSidebar.themePanel.fonts.sans",
                              "Modern (Sans-serif)",
                            )
                          : f === "serif-sans"
                            ? t(
                                "editorSidebar.themePanel.fonts.serifSans",
                                "Elegant (Serif + Sans)",
                              )
                            : t(
                                "editorSidebar.themePanel.fonts.serif",
                                "Classic (Serif)",
                              )}
                      </button>
                    ),
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <p className="text-xs font-medium text-stone-500">
                    {t("editorSidebar.themePanel.radius", "Border Radius")}
                  </p>
                  <span className="text-xs text-stone-400">
                    {config.theme.radius}px
                  </span>
                </div>
                <input
                  type="range"
                  ...
                  onChange={(e) =>
                    patchTheme({ radius: parseInt(e.target.value) })
                  }
              ...
              <div>
                <p className="text-xs font-medium text-stone-500 mb-2">
                  {t("editorSidebar.themePanel.buttonStyle.title")}
                </p>
                <div className="flex gap-2">
              ...
              <div>
                <p className="text-xs font-medium text-stone-500 mb-2">
                  {t("editorSidebar.themePanel.animation.title")}
                </p>
              ...
              <div>
                <p className="text-xs font-medium text-stone-500 mb-2">
                  {t("editorSidebar.themePanel.pageWidth", "Page Width")}
                </p>
                <div className="flex gap-2">
                  {(["contained", "wide", "full"] as const).map((w) => (
                    <button
                      key={w}
                      onClick={() => patchTheme({ pageWidth: w })}
              ...
              <div>
                <p className="text-xs font-medium text-stone-500 mb-2">
                  {t("editorSidebar.themePanel.cardShadow", "Card Shadow")}
                </p>
                <div className="flex gap-2">
                  {(["none", "soft", "strong"] as const).map((s) => (
                    <button
                      key={s}
                      onClick={() => patchTheme({ cardShadow: s })}
