...
export default function PublishReviewModal({
  ...
  const issues = Object.values(readiness).filter(
    (item) => !item.done && item.severity === "required",
  );
  const totalSteps = Object.values(readiness).filter(
        ...
        <div className="p-6 overflow-y-auto space-y-6">
          <div className="text-center">
            ...
            <h3 className="text-xl font-bold text-stone-800 mb-2">
              {isReady
                ? t(
                    "publishReview.readyTitle",
                    "Your store is ready for the world!",
                  )
                : t(
                    "publishReview.notReadyTitle",
                    "Your store story is almost complete",
                  )}
            </h3>
            <p className="text-sm text-stone-500">
              {isReady
                ? t(
                    "publishReview.readyDesc",
                    "All sections are fully configured. You are good to go.",
                  )
                : t(
                    "publishReview.notReadyDesc",
                    "We found a few missing pieces in your store's story. It's highly recommended to complete them before publishing.",
                  )}
            </p>
          </div>
          ...
          {!isReady && (
            <div className="space-y-3">
              <div className="flex items-center justify-between text-sm font-semibold text-stone-700">
                <span>
                  {t("publishReview.readinessScore", "Readiness Score")}
                </span>
              ...
              <div className="mt-4 space-y-2">
                {issues.map((issue, idx) => (
                  <div
                    key={idx}
                    className="flex gap-3 items-start p-3 rounded-xl bg-rose-50 border border-rose-100/50"
                  >
                    <AlertCircle className="w-4 h-4 text-rose-500 mt-0.5 shrink-0" />
                    <div>
                      <p className="text-xs font-semibold text-stone-800">
                        {t(`launchReadiness.step.${issue.key}`)}
                      </p>
                      <p className="text-[11px] text-stone-500 mt-0.5">
                        {t(`launchReadiness.desc.${issue.key}`)}
                      </p>
                    </div>
                  </div>
                ))}
