...
export function SectionPreview({ section, theme, brand, categories = [], selected, onClick }: PreviewProps) {
  ...
  switch (section.type) {
      ...
      return wrap(
        <div className="bg-white px-6 py-8">
          ...
          {previewCategories.length > 0 ? (
            ...
            <div className="grid grid-cols-4 gap-3">
              {(t("sectionPreview.defaults.categoriesDummy", { returnObjects: true }) as Array<{ name: string; image: string }>).map((cat, i) => (
                <div key={i} className="relative overflow-hidden bg-stone-100" style={{ borderRadius: r }}>
                  <img src={cat.image} alt={cat.name} className="aspect-square w-full object-cover" loading="lazy" />
                  <div className="absolute inset-0 flex items-end p-2 bg-gradient-to-t from-black/55 to-transparent">
    ...
    case "offers": {
      ...
      if (!showPromo1 && !showPromo2) return wrap(<div className="py-8 text-center text-xs text-stone-400">No offers visible</div>);
      return wrap(
        <div className={`grid grid-cols-1 ${showPromo1 && showPromo2 ? "grid-cols-2 gap-2" : ""} p-4 bg-white`}>
          {showPromo1 && (
            <div className="px-4 py-6 text-center text-white relative overflow-hidden rounded-xl" style={{ background: `linear-gradient(135deg, ${p}f0, #c97b8b)` }}>
              ...
              <p className="text-sm font-bold mb-1" style={{ fontFamily: "'Cormorant Garamond', serif" }}>
                {(section.content.promo1Heading as string) ?? t("sectionPreview.defaults.offersHeading")}
              </p>
              <span
                className="bg-white/20 text-[10px] px-2 py-1 font-semibold inline-block rounded"
              >
                {(section.content.promo1Cta as string) ?? t("sectionPreview.defaults.offersCta")}
              </span>
            </div>
          )}
          {showPromo2 && (
        ...
        <div className="bg-white px-6 py-8">
          ...
          <div className="grid gap-2" style={{ gridTemplateColumns: `repeat(${cols}, 1fr)` }}>
            {displayItems.map((item, i) => {
              const bgStyle = !item.imageUrl ? { background: `${i % 2 === 0 ? p : sec}${15 + i * 5}` } : {};
              return (
                <div
                  key={i}
