...
const SAMPLE_COMMENTS: SocialComment[] = [
  { id: "1", platform: "tiktok",    username: "@sara_cairo",   comment: "عايزة اطلب المنتج ده! كام سعره؟",        productHint: "المنتج الأول",   timestamp: new Date(Date.now() - 1000 * 60 * 5).toISOString(),  converted: false },
  { id: "2", platform: "instagram", username: "@noura.style",  comment: "كيف اطلب؟ في توصيل القاهرة؟",            productHint: "الفستان الأحمر", timestamp: new Date(Date.now() - 1000 * 60 * 15).toISOString(), converted: false },
  { id: "3", platform: "tiktok",    username: "@mona_fashion", comment: "ممكن رابط الطلب",                        productHint: "الحقيبة الجلدية",timestamp: new Date(Date.now() - 1000 * 60 * 30).toISOString(), converted: true  },
  { id: "4", platform: "instagram", username: "@layla_beauty", comment: "بكام القطعة دي؟ وفيه ألوان تانية؟",     productHint: "كريم العناية",   timestamp: new Date(Date.now() - 1000 * 60 * 60).toISOString(), converted: false },
];
...
export default function SocialOrders() {
  ...
  const convertMutation = useMutation({
    mutationFn: async ({ comment, formData }: { comment: SocialComment; formData: typeof form }) => {
      ...
      if (!product) throw new Error("اختر منتجاً أولاً");
    ...
    onSuccess: (_, { comment }) => {
      toast({ title: "تم إنشاء الطلب", description: "تم تحويل التعليق إلى طلب بنجاح" });
    ...
    <div className="space-y-6 pb-10" dir="rtl">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold flex items-center gap-2">
            ...
            طلبات التعليقات — TikTok / Instagram
          </h1>
          <p className="text-sm text-muted-foreground mt-1">حوّل تعليقات المتابعين إلى طلبات مباشرة بضغطة واحدة</p>
        </div>
        <Badge className="bg-primary/10 text-primary border-primary/20 shrink-0">
          {pendingCount} تعليق جديد
        </Badge>
      </div>
      ...
      <Card className="border-border/50 bg-gradient-to-br from-pink-50/50 to-purple-50/50 dark:from-pink-950/20 dark:to-purple-950/20">
        <CardContent className="p-4">
          <p className="text-xs font-medium mb-3 text-muted-foreground">كيف تعمل الميزة؟</p>
          <div className="grid grid-cols-3 gap-3 text-center text-[11px] text-muted-foreground">
            {[
              { icon: MessageCircle, label: "يعلّق المتابع على المنتج", color: "text-pink-500" },
              { icon: Zap,           label: "يظهر التعليق هنا فوراً",   color: "text-purple-500" },
              { icon: ShoppingCart,  label: "تحوّله لطلب بضغطة واحدة", color: "text-green-500" },
            ].map((step, i) => (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {comment.converted && <Badge className="bg-green-100 text-green-700 text-[10px]">تم التحويل</Badge>}
                    </div>
                  </div>
                ...
                convertingId === comment.id ? (
                  <div className="border-t border-border/50 pt-3 space-y-2">
                    ...
                    >
                      ...
                      {(products ?? []).map((p: any) => (
                        <option key={p.id} value={p.id}>{p.name} — {parseFloat(p.price).toLocaleString("ar-EG")} ج.م</option>
                      ))}
                    </select>
                    <div className="flex gap-2">
                      ...
                      >
                        {convertMutation.isPending ? "جاري الإنشاء..." : "إنشاء الطلب"}
                      </Button>
      ...
      <Card className="border-dashed border-2 border-border/50">
        <CardContent className="p-4 text-center space-y-2">
          ...
          <p className="text-sm font-medium">ربط حسابات التواصل الاجتماعي</p>
          <p className="text-xs text-muted-foreground">اربط صفحة Instagram أو حساب TikTok لاستيراد التعليقات تلقائياً وتحويلها لطلبات فورية</p>
          <Button variant="outline" size="sm" className="text-xs" disabled>
            قريباً — Instagram API
          </Button>
        </CardContent>
      </Card>
    </div>
