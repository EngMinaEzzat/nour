...
const STATUS_MAP: Record<string, { label: string; color: string }> = {
  ...
  shipped:   { label: "تم الشحن",     color: "bg-purple-100 text-purple-800 border-purple-200" },
...
const stagger = {
  ...
  item: { hidden: { opacity: 0, x: -10 }, show: { opacity: 1, x: 0, transition: { duration: 0.3 } } },
};
...
export default function Orders() {
  ...
  const filtered = orders?.filter((o) => {
    const matchesSearch = !search || String(o.id).includes(search) || o.customerName?.toLowerCase().includes(search.toLowerCase());
    ...
    <div className="container mx-auto px-4 py-10">
      ...
      >
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <Button size="sm" variant={!statusFilter ? "default" : "outline"} className="rounded-full" onClick={() => setStatusFilter(null)}>الكل</Button>
          {Object.entries(STATUS_MAP).map(([key, val]) => (
            <Button key={key} size="sm" variant={statusFilter === key ? "default" : "outline"} className="rounded-full" onClick={() => setStatusFilter(statusFilter === key ? null : key)}>
              {val.label}
            </Button>
          ))}
        </div>
      ...
      {!isLoading && <p className="text-sm text-muted-foreground mb-4">{filtered?.length ?? 0} طلب</p>}
                    ...
                    <div className="flex items-center justify-between gap-4">
                      ...
                      <div className="flex items-center gap-4">
                        <div className="text-end hidden sm:block">
                          <p className="font-bold text-primary">{Number(order.totalAmount ?? 0).toLocaleString("ar-EG")} ج.م</p>
