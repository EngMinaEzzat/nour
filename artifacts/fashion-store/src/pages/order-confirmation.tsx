    ...
    <div className="container mx-auto px-4 py-16 max-w-2xl text-center">
      ...
      >
        ...
        <p className="text-lg text-muted-foreground mb-2">
          شكرًا لكِ، <span className="text-foreground font-semibold">{decodeURIComponent(name)}</span>
        </p>
        <p className="text-muted-foreground mb-4">
          {paymentMethod === "paymob"
            ? "تم استلام دفعتك بنجاح. سيتم تجهيز طلبك وشحنه في أقرب وقت."
            : "استلمنا طلبك وسيتم تجهيزه في أقرب وقت. ستصلكِ تفاصيل التوصيل قريبًا."}
        </p>

        {/* Payment badge */}
        <div className="inline-flex items-center gap-2 bg-muted px-4 py-2 rounded-full text-sm text-muted-foreground mb-8">
          {paymentMethod === "paymob"
            ? <><span>💳</span> تم الدفع إلكترونياً عبر Paymob</>
          ...
          <Card className="border-border/50 mb-6 text-start">
            <CardContent className="pt-6 space-y-3">
              <div className="flex items-center gap-2 text-sm font-semibold text-muted-foreground mb-4">
                ...
                {orderIds.length === 1 ? "رقم الطلب" : "أرقام الطلبات"}
              </div>
              {orderIds.map((id) => (
                ...
                <CardContent className="pt-5 pb-4">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div className="text-start">
                      <p className="font-semibold text-sm text-emerald-800 dark:text-emerald-300">احصلي على تأكيد الطلب على واتساب</p>
                      <p className="text-xs text-muted-foreground mt-0.5">اضغطي لفتح رسالة التأكيد على {phone}</p>
                    </div>
                  </div>
                  <Button
            ...
            <Card className="border-blue-200/60 bg-blue-50/30 dark:border-blue-800/30 dark:bg-blue-900/10 mb-8">
              <CardContent className="pt-5 pb-4 text-start">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-semibold text-sm text-blue-800 dark:text-blue-300">التوصيل عبر بوسطة</p>
                    <p className="text-xs text-muted-foreground mt-0.5">
                      سيقوم المتجر بإنشاء شحنتكِ خلال 24 ساعة — ستصلكِ رسالة برقم التتبع
                    </p>
                  </div>
                </div>
              </CardContent>
      ...
      >
        ...
        <Button asChild variant="outline" className="rounded-full px-8">
          <Link href="/"><Home className="w-4 h-4 me-2" /> الصفحة الرئيسية</Link>
        </Button>
      </motion.div>
    </div>
  );
