...
| "modern-boutique"
  ...
  | "local-brand"
  | "playful-shop"
  | "luxury-catalog"  | "streetwear-cyberpunk"  | "imperial-chic";
...
export function createDefaultSection(type: SectionType, storeName: string, category: string = "fashion", t?: TFunction, selectedStyle?: StyleType): SectionConfig {
  ...
  const isImperial = selectedStyle === "imperial-chic";
  ...
  const defaults: Record<SectionType, { content: SectionContent; settings: SectionSettings }> = {
    hero: {
      content: { 
        heading: tr(isImperial ? "defaultSections.imperialChic.hero.heading" : isCyberpunk ? "defaultSections.streetwearCyberpunk.hero.heading" : isCosmetics ? "defaultSections.hero.headingCosmetics" : "defaultSections.hero.heading", isImperial ? `المجموعة الإمبراطورية في ${storeName}` : isCyberpunk ? `Own the Streets with ${storeName}` : isCosmetics ? `اكتشفي جمالكِ مع ${storeName}` : `اكتشفي أحدث تشكيلة من ${storeName}`, { storeName }),
        subheading: tr(isImperial ? "defaultSections.imperialChic.hero.subheading" : isCyberpunk ? "defaultSections.streetwearCyberpunk.hero.subheading" : isCosmetics ? "defaultSections.hero.subheadingCosmetics" : "defaultSections.hero.subheading", isImperial ? "أناقة القصور وعظمة كلاسيكية خالدة" : isCyberpunk ? "Cyber Drip & High-Energy Aesthetics" : isCosmetics ? "مستحضرات عناية وتجميل تبرز جمالك الطبيعي" : "أزياء راقية بأسعار تناسبك"),
        ctaText: tr(isImperial ? "defaultSections.imperialChic.hero.ctaText" : isCyberpunk ? "defaultSections.streetwearCyberpunk.hero.ctaText" : "defaultSections.hero.ctaText", isImperial ? "شاهدي المجموعة" : isCyberpunk ? "Hack the System" : "تسوقي الآن"),
        ctaLink: "#products",
        imageUrl: isImperial ? "/hero-imperial-optimized.jpg" : isCyberpunk ? "/hero-streetwear-optimized.jpg" : undefined
      },
    ...
    about: {
      content: { 
        heading: tr(isImperial ? "defaultSections.imperialChic.about.heading" : isCyberpunk ? "defaultSections.streetwearCyberpunk.about.heading" : "defaultSections.about.heading", isImperial ? "روعة خالدة" : isCyberpunk ? `The Core of ${storeName}` : `قصة ${storeName}`, { storeName }),
        body: tr(isImperial ? "defaultSections.imperialChic.about.body" : isCyberpunk ? "defaultSections.streetwearCyberpunk.about.body" : isCosmetics ? "defaultSections.about.bodyCosmetics" : "defaultSections.about.bodyFashion", isImperial ? "مستوحاة من العصور القديمة ومصممة للعصر الحديث. نقدم لكِ درجات العنابي العميق، الذهب العتيق، وجمالية مهيبة تتحدى الزمن." : isCyberpunk ? "Born in the neon glow, forged in the concrete jungle. We bring you the ultimate streetwear aesthetic, blending brutalist design with high-octane energy. Own your narrative." : isCosmetics ? "نؤمن بأن الجمال الحقيقي ينبع من الداخل، ومهمتنا هي توفير أفضل مستحضرات العناية والتجميل لتعزيز ثقتكِ بنفسكِ. كل منتج نختاره بعناية ليناسب احتياجاتكِ." : "نؤمن بأن كل امرأة تستحق أن تشعر بالثقة والأناقة. بدأنا رحلتنا بشغف حقيقي لتقديم أجمل الأزياء بأفضل الأسعار."),
        imageUrl: isImperial ? "/about-imperial-optimized.jpg" : isCyberpunk ? "/about-streetwear-optimized.jpg" : "/about-optimized.jpg"
      },
    ...
    lookbook: {
      content: { 
        heading: tr(isImperial ? "defaultSections.imperialChic.lookbook.heading" : "defaultSections.lookbook.heading", isImperial ? "القاعة الرخامية" : "لوك بوك - إلهامي هذا الموسم"),
        items: tr(isImperial ? "defaultSections.imperialChic.lookbook.items" : "defaultSections.lookbook.items", isImperial ? [
          {
            imageUrl: "/lookbook-1-imperial-optimized.jpg",
            tag: "Cosmetics",
            title: "عنابي\n عميق",
            desc: "درجات غنية وآسرة.",
            categoryId: "",
          },
          {
            imageUrl: "/lookbook-2-imperial-optimized.jpg",
            tag: "Fashion",
            title: "ذهب\n عتيق",
            desc: "لمسات معدنية عتيقة.",
            categoryId: "",
          },
          {
            imageUrl: "/lookbook-3-imperial-optimized.jpg",
            tag: "Still Life",
            title: "بشرة\n المرمر",
            desc: "جمال نقي وخالٍ من العيوب.",
            categoryId: "",
          },
        ] : [
          {
    ...
    "trust-strip": {
      content: { 
        items: tr(isImperial ? "defaultSections.imperialChic.trustStrip.items" : isCyberpunk ? "defaultSections.streetwearCyberpunk.trustStrip.items" : "defaultSections.trustStrip.items", isImperial ? [
          { icon: "🏛️", title: "إرث كلاسيكي", text: "مستوحى من العظماء" },
          { icon: "📜", title: "جودة تاريخية", text: "صُنعت لتبقى في الذاكرة" },
          { icon: "⚖️", title: "استبدال عادل", text: "سياسة إرجاع متوازنة" },
          { icon: "🕊️", title: "خدمة راقية", text: "في خدمتك دائماً" }
        ] : isCyberpunk ? [
...
export const PERSONALITY_PRESETS: Record<PersonalityType, { label: string; desc: string; emoji: string; colors: string[]; font: string; example: string; theme: Partial<ThemeConfig> }> = {
  imperial: {
    label: "إمبراطورية فاخرة", desc: "تصميم ملكي مستوحى من القصور الرومانية واليونانية", emoji: "🏛️",
    colors: ["#800020", "#cfb53b"], font: "Trajan Pro",
    example: "\"أناقة القصور وعظمة كلاسيكية خالدة\"",
    theme: { primaryColor: "#800020", secondaryColor: "#cfb53b", fontPairing: "serif-serif", buttonStyle: "rounded", radius: 4, cardShadow: "soft" },
  },
  elegant: {
