...
import {
  request, app, uid, createTestMerchant, cleanupTenant, createTestProduct, createTestOrder,
} from "./helpers.js";
...
describe("Edge Cases — Product Validation", () => {
  ...
  it("✅ product name at maximum length (255 chars) succeeds", async () => {
    ...
    const res = await createTestProduct(ctx.agent, { name, price: 100, stock: 1 });
    expect([200, 201]).toContain(res.status);
  });

  it("✅ product with price=0 is rejected or accepted consistently", async () => {
    const res = await createTestProduct(ctx.agent, { name: "Free Item", price: 0, stock: 10 });
  ...
  it("✅ product with very large stock number is handled", async () => {
    const res = await createTestProduct(ctx.agent, { name: "Huge Stock", price: 100, stock: 999999 });
  ...
  it("❌ product with missing name returns 400", async () => {
    const res = await ctx.agent.post("/api/products").send({ price: 100, stock: 10, status: "active" });
    expect([400, 422]).toContain(res.status);
  });

  it("❌ update non-existent product returns 404", async () => {
    const res = await ctx.agent.put("/api/products/9999999").send({ price: 200 });
...
describe("Edge Cases — Order Validation", () => {
  ...
  beforeAll(async () => {
    ...
    const p = await createTestProduct(ctx.agent, { name: "Order Test Product", price: 199, stock: 10 });
  ...
  beforeAll(async () => {
    ...
    await Promise.all(
      Array.from({ length: 15 }, (_, i) =>
        createTestProduct(ctx.agent, { name: `Bulk Product ${i}`, price: 50 + i, stock: i + 1 })
      )
    );
  });
  afterAll(async () => { await cleanupTenant(ctx.tenantId, ctx.merchantId); });
