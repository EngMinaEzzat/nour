...
describe("Audit Routes", () => {
  ...
  afterAll(async () => {
    if (merchantCtx)
      await cleanupTenant(merchantCtx.tenantId, merchantCtx.merchantId);
  ...
  describe("GET /api/audit/events/my", () => {
    it("✅ should get own tenant audit events", async () => {
      ...
      expect(res.body.map((e: any) => e.eventType)).toEqual(
        expect.arrayContaining(["product_deleted", "plan_changed"]),
      );
    });
  ...
  describe("GET /api/audit/events", () => {
    it("✅ platform admin sees all events without filters", async () => {
      ...
      const myTenantEvents = events.filter(
        (e: any) => e.tenantId === merchantCtx.tenantId,
      );
      const otherTenantEvents = events.filter(
        (e: any) => e.tenantId === otherCtx.tenantId,
      );
    ...
    it("✅ platform admin can filter by tenantId", async () => {
      const res = await adminCtx.agent.get(
        `/api/audit/events?tenantId=${merchantCtx.tenantId}`,
      );
      expect(res.status).toBe(200);
      const events = res.body;
      ...
      expect(
        events.every((e: any) => e.tenantId === merchantCtx.tenantId),
      ).toBe(true);
    });

    it("✅ platform admin can filter by eventType", async () => {
      const res = await adminCtx.agent.get(
        `/api/audit/events?eventType=plan_changed`,
      );
      expect(res.status).toBe(200);
      const events = res.body;

      expect(events.length).toBe(1);
      expect(events.every((e: any) => e.eventType === "plan_changed")).toBe(
        true,
      );
    });
    ...
    it("✅ platform admin can filter by both tenantId and eventType", async () => {
      const res = await adminCtx.agent.get(
        `/api/audit/events?tenantId=${merchantCtx.tenantId}&eventType=product_deleted`,
      );
      expect(res.status).toBe(200);
      const events = res.body;
      ...
      expect(
        events.every(
          (e: any) =>
            e.tenantId === merchantCtx.tenantId &&
            e.eventType === "product_deleted",
        ),
      ).toBe(true);
    });
