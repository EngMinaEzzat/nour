...
describe("AI Assistant — Chat Endpoint", () => {
  ...
  it("❌ unauthenticated request returns 401", async () => {
    const res = await request(app)
      .post("/api/ai/assistant/chat")
      .send({ message: "hello", model: "gemini" });
  ...
  it("✅ unknown model name falls back to gemini gracefully (not 500)", async () => {
    const res = await ctx.agent
      .post("/api/ai/assistant/chat")
  ...
  it("✅ defaults to gemini model when model not specified", async () => {
    const res = await ctx.agent
      .post("/api/ai/assistant/chat")
      .send({ message: "مرحباً" });
    expect([200, 400, 429, 503]).toContain(res.status);
...
describe("AI Assistant — Rate Limiting", () => {
  ...
  it("✅ endpoint is accessible to authenticated merchants", async () => {
    const res = await ctx.agent
      .post("/api/ai/assistant/chat")
      .send({ message: "اختبار الوصول", model: "gemini" });
  ...
  it("✅ rate limit response includes proper structure when limit hit", async () => {
    ...
    for (let i = 0; i < 3; i++) {
      const res = await ctx.agent
        .post("/api/ai/assistant/chat")
        .send({ message: `رسالة اختبار ${i}`, model: "gemini" });
      if (res.status === 429) {
        rateLimitHit = true;
