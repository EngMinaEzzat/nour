...
describe("Customers — Create (public)", () => {
  ...
  it("✅ POST /customers with existing email returns existing record (idempotent)", async () => {
    ...
    const r1 = await request(app).post("/api/customers").send({ name: "First", phone: "01011111111", email });
    const r2 = await request(app).post("/api/customers").send({ name: "Second", phone: "01022222222", email });
