...
import { eq, inArray, and, sql, lt, count } from "drizzle-orm";
...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // Pre-fetch all relevant contact attempts for orders that need them to avoid N+1
      ...
      // Map of orderId -> ContactAttempt[]
      const contactAttemptsMap = new Map<
        number,
        { method: string; createdAt: Date }[]
      >();

      if (contactOrderIds.length > 0) {
        const allAttempts = await db
          .select({
            orderId: contactAttemptsTable.orderId,
            method: contactAttemptsTable.method,
            createdAt: contactAttemptsTable.createdAt,
          })
          .from(contactAttemptsTable)
          .where(inArray(contactAttemptsTable.orderId, contactOrderIds))
          .orderBy(sql`${contactAttemptsTable.createdAt} DESC`);

        for (const attempt of allAttempts) {
          if (!contactAttemptsMap.has(attempt.orderId)) {
            contactAttemptsMap.set(attempt.orderId, []);
          }
          contactAttemptsMap.get(attempt.orderId)!.push(attempt);
        }
      }
      ...
      for (const order of allOrders) {
        ...
        if (
          ...
          const attemptsForOrder = contactAttemptsMap.get(order.id) || [];
          const attemptCount = attemptsForOrder.length;
          ...
          const lastAttempts =
            attemptsForOrder.length > 0 ? [attemptsForOrder[0]] : [];
