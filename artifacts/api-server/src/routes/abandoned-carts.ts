...
router.post("/cart/contact", async (req, res) => {
  const { sessionId, tenantId, customerName, customerEmail, customerPhone } = req.body;
  if (!sessionId || !tenantId) return res.status(400).json({ error: "sessionId and tenantId required" });
...
router.get("/abandoned-carts", requireRole("owner", "manager"), async (req, res) => {
  ...
  try {
    ...
    const [stats] = await db
      ...
      .from(cartSessionsTable)
      .where(and(eq(cartSessionsTable.tenantId, tenantId), ne(cartSessionsTable.status, "converted")));
...
router.post("/abandoned-carts/:id/remind", requireRole("owner", "manager", "staff"), async (req, res) => {
  ...
  try {
    const [cart] = await db
      ...
      .from(cartSessionsTable)
      .where(and(eq(cartSessionsTable.id, id), eq(cartSessionsTable.tenantId, tenantId)));

    if (!cart) return res.status(404).json({ error: "السلة غير موجودة" });
    if (!cart.customerPhone) return res.status(400).json({ error: "لا يوجد رقم واتساب للعميل" });
    ...
    const total = parseFloat(cart.totalAmount as string).toLocaleString("ar-EG");
    const storeName = tenant?.name ?? "متجرنا";
    const storeUrl = tenant?.slug ? `https://nour.replit.app/store/${tenant.slug}` : "";
