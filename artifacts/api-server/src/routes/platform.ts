...
import {
  tenantsTable, productsTable, ordersTable, merchantOnboardingTable,
...
router.get("/platform/stats", requirePlatformAdmin, async (req, res) => {
  try {
    ...
    const trialTenants = allTenants.filter((t) => t.subscriptionStatus === "trial").length;
    const suspendedTenants = allTenants.filter((t) =>
      t.subscriptionStatus === "suspended" || t.subscriptionStatus === "past_due"
    ).length;
    ...
    const planBreakdown = Object.entries(planBreakdownMap).map(([planCode, cnt]) => ({
      planCode,
      count: cnt,
    }));
...
router.get("/platform/provider-health", requirePlatformAdmin, async (req, res) => {
  try {
    const providers = await db
      ...
      .leftJoin(tenantsTable, eq(whatsappProvidersTable.tenantId, tenantsTable.id))
    ...
    const withStats = await Promise.all(
      providers.map(async (p) => {
        ...
        return {
          ...
          recentLogs: recent.map((r) => ({ ...r, createdAt: r.createdAt.toISOString() })),
        };
      })
    );
...
router.put("/platform/provider-health/:tenantId/disable", requirePlatformAdmin, async (req, res) => {
...
router.get("/platform/health-scores", requirePlatformAdmin, async (req, res) => {
  try {
    ...
    const productCounts = await db.select({ tenantId: productsTable.tenantId, cnt: count() }).from(productsTable).groupBy(productsTable.tenantId);
    const orderCounts = await db.select({ tenantId: ordersTable.tenantId, cnt: count() }).from(ordersTable).groupBy(ordersTable.tenantId);
    ...
    const scores = allTenants.map((t) => {
      ...
      const daysSinceCreated = Math.floor((Date.now() - new Date(t.createdAt).getTime()) / 86400000);

      let score = 0;
      const signals: string[] = [];

      if (t.subscriptionStatus === "active") { score += 30; signals.push("اشتراك نشط"); }
      else if (t.subscriptionStatus === "trial") { score += 10; signals.push("فترة تجريبية"); }

      if (pc >= 10) { score += 25; signals.push(`${pc} منتج`); }
      else if (pc > 0) { score += 10; signals.push(`${pc} منتج`); }
      else signals.push("لا توجد منتجات");

      if (oc >= 20) { score += 30; signals.push(`${oc} طلب`); }
      else if (oc >= 5) { score += 20; signals.push(`${oc} طلب`); }
      else if (oc > 0) { score += 10; signals.push(`${oc} طلب`); }
      else signals.push("لا توجد طلبات");

      if (daysSinceCreated > 30 && oc === 0) score -= 10;

      const healthLabel = score >= 70 ? "ممتاز" : score >= 40 ? "جيد" : score >= 20 ? "ضعيف" : "حرج";
      return { tenantId: t.id, name: t.name, slug: t.slug, planCode: t.planCode, subscriptionStatus: t.subscriptionStatus, score: Math.max(0, Math.min(100, score)), healthLabel, signals, productCount: pc, orderCount: oc };
    });
...
router.put("/platform/merchants/:tenantId/status", requirePlatformAdmin, async (req, res) => {
  const tenantId = Number(req.params.tenantId);
  if (isNaN(tenantId)) return res.status(400).json({ error: "معرّف المتجر غير صحيح" });
  ...
  try {
    const [updated] = await db
      ...
      .returning({ id: tenantsTable.id, subscriptionStatus: tenantsTable.subscriptionStatus });
    ...
    if (action === "suspend") {
      const [owner] = await db
        ...
        .where(and(eq(merchantsTable.tenantId, tenantId), eq(merchantsTable.role, "owner")));

      if (owner?.email) {
        const baseUrl = process.env.APP_BASE_URL ?? "https://nour.eg";
        sendSubscriptionSuspendedEmail(owner.email, owner.name, `${baseUrl}/billing`)
...
router.get("/platform/transfer-requests", requirePlatformAdmin, async (req, res) => {
  try {
    const rows = await db
      .select({
      ...
      .leftJoin(tenantsTable, eq(billingTransferRequestsTable.tenantId, tenantsTable.id))
      .leftJoin(merchantsTable, and(
...
router.put("/platform/transfer-requests/:id/approve", requirePlatformAdmin, async (req, res) => {
  const id = Number(req.params.id);
  if (isNaN(id)) return res.status(400).json({ error: "معرّف غير صحيح" });

  try {
    const [request] = await db.select().from(billingTransferRequestsTable)
      .where(eq(billingTransferRequestsTable.id, id));
    if (!request) return res.status(404).json({ error: "الطلب غير موجود" });
    if (request.status !== "pending") return res.status(409).json({ error: "الطلب تمت مراجعته مسبقًا" });

    const [tenant] = await db.select({
      subscriptionStatus: tenantsTable.subscriptionStatus,
      trialEndsAt: tenantsTable.trialEndsAt,
      subscriptionStartedAt: tenantsTable.subscriptionStartedAt,
    }).from(tenantsTable).where(eq(tenantsTable.id, request.tenantId));
    ...
    if (tenant.subscriptionStatus === "trial" && tenant.trialEndsAt && tenant.trialEndsAt > now) {
      ...
      const prevEnd = new Date(tenant.subscriptionStartedAt.getTime() + MONTH_MS);
    ...
    await db.transaction(async (tx) => {
      await tx.update(billingTransferRequestsTable).set({
        status: "approved",
    ...
    res.json({ success: true, periodStart: periodStart.toISOString(), periodEnd: periodEnd.toISOString() });
  } catch (err) {
    req.log.error(err);
...
router.put("/platform/transfer-requests/:id/reject", requirePlatformAdmin, async (req, res) => {
  const id = Number(req.params.id);
  if (isNaN(id)) return res.status(400).json({ error: "معرّف غير صحيح" });
  const { note } = req.body as { note?: string };

  try {
    const [request] = await db.select({ id: billingTransferRequestsTable.id, status: billingTransferRequestsTable.status })
      .from(billingTransferRequestsTable).where(eq(billingTransferRequestsTable.id, id));
    if (!request) return res.status(404).json({ error: "الطلب غير موجود" });
    if (request.status !== "pending") return res.status(409).json({ error: "الطلب تمت مراجعته مسبقًا" });

    await db.update(billingTransferRequestsTable).set({
      status: "rejected",
      adminNote: note ?? null,
      reviewedBy: req.session.merchantId,
      reviewedAt: new Date(),
